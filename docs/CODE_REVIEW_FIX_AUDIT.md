...
| CR-010/CR-016 | builtins history metadata / sema history / runtime registry / SAR+DMI+Supertrend numeric baselines | 🟡 | 已引入 `BUILTIN_HISTORY_METADATA`、由 sema 消费，补 reviewed runtime/metadata 对账，并把 `ta.sar`/`ta.dmi`/`ta.supertrend` 数值基准绑定到各自 HIR retention；其他高风险指标数值基准仍待扩展。 |
