...
| CR-010/CR-016 | builtins history metadata / sema history / runtime registry / SAR numeric baseline | 🟡 | 已引入 `BUILTIN_HISTORY_METADATA`、由 sema 消费，补 reviewed runtime/metadata 对账，并把 `ta.sar` 数值基准绑定到 `high[2]/low[2]/close[1]` retention；其他高风险指标数值基准仍待扩展。 |
