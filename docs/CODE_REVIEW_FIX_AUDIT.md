...
| CR-010/CR-016 | builtins history metadata / sema history / runtime registry / SAR+DMI+Supertrend+KC/KCW+MFI/TSI+Cross numeric baselines | 🟡 | 已引入 `BUILTIN_HISTORY_METADATA`、由 sema 消费，补 reviewed runtime/metadata 对账，并把 `ta.sar`/`ta.dmi`/`ta.supertrend`/`ta.kc`/`ta.kcw`/`ta.mfi`/`ta.tsi`/`ta.cross*` 数值基准绑定到各自 HIR retention；runtime helper/debug assertion closeout 仍待评估。 |
