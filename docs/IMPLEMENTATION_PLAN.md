...
`ta.variance`, `ta.range`, `ta.highest`, and `ta.lowest`. These functions now
maintain per-callsite rolling windows instead of allocating a fresh window from
committed history on every bar.
