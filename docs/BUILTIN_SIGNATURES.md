...
entries with unique entry ids until the position closes. Omitted-`from_entry`
full trailing exits (`trail_price+trail_offset` and
...
entry ids are unique. Omitted-`from_entry` `stop+profit`, `loss+limit`,
`stop+limit`, and trailing relative future-entry persistence and duplicate
