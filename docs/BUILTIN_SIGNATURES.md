...
- `request.security("SYMBOL", timeframe, expression)` and
  ...
  stateless `math.*` calls, `math.sum`, `ta.sma`, and `ta.ema`.
...
stateful/contextual math calls such as `math.random` and
`math.round_to_mintick`, output/drawing side effects, input declarations, array
mutation, optional parameters, non-default barmerge behavior, explicit gaps, and
lookahead remain unsupported.
