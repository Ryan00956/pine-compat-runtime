...
exact bound matrix-valued `matrix_id.mult(other)` results share the thirty
matrix helpers for matrix or scalar operands with the
copy/eigenvectors/inv/pinv/pow/submatrix/transpose/row/column/eigenvalue/predicate/aggregate-reader continuation
rules.
...
rejection. Producer-specific “copy/eigenvectors/inv/pinv/pow/submatrix/transpose-only” wording below refers only
...
helpers, add `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)` for numeric results, and retain
copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation after the original receiver resolves
...
kind helpers, add `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)` for numeric results, and retain
copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation after the original receiver resolves
...
numeric-matrix-receiver `matrix_id.kron(other)` results share the same eleven
read/copy/eigenvectors/inv/pinv/pow/submatrix/transpose helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose
...
numeric-matrix-receiver `matrix_id.diff(other)` results share the eleven
read/copy/eigenvectors/inv/pinv/pow/submatrix/transpose helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose
...
semantics. Exact bound `matrix_id.pow(power)` results share the eleven matrix
helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation.
...
share the eleven matrix helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation.
Exact namespace
...
`matrix_id.pinv()` results share the eleven matrix helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose
...
share the eleven matrix helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation. Exact
...
template results additionally expose `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)`. Map templates such as
...
numeric `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)` added after the receiver-kind check and
copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation; they do not widen imported UDT
identity.
