...
exact bound matrix-valued `matrix_id.mult(other)` results share the twenty-nine
matrix helpers for matrix or scalar operands with the
copy/eigenvectors/inv/pinv/submatrix/transpose/row/column/eigenvalue/predicate/aggregate-reader continuation
rules.
...
rejection. Producer-specific “copy/eigenvectors/inv/pinv/submatrix/transpose-only” wording below refers only
...
helpers, add `.eigenvectors()`, `.inv()`, and `.pinv()` for numeric results, and retain
copy/eigenvectors/inv/pinv/submatrix/transpose continuation after the original receiver resolves
...
kind helpers, add `.eigenvectors()`, `.inv()`, and `.pinv()` for numeric results, and retain
copy/eigenvectors/inv/pinv/submatrix/transpose continuation after the original receiver resolves
...
numeric-matrix-receiver `matrix_id.kron(other)` results share the same ten
read/copy/eigenvectors/inv/pinv/submatrix/transpose helpers and copy/eigenvectors/inv/pinv/submatrix/transpose
...
numeric-matrix-receiver `matrix_id.diff(other)` results share the nine
read/copy/eigenvectors/inv/pinv/submatrix/transpose helpers and copy/eigenvectors/inv/pinv/submatrix/transpose
...
semantics. Exact bound `matrix_id.pow(power)` results share the ten matrix
helpers and copy/eigenvectors/inv/pinv/submatrix/transpose continuation.
...
share the ten matrix helpers and copy/eigenvectors/inv/pinv/submatrix/transpose continuation.
Exact namespace
...
`matrix_id.pinv()` results share the ten matrix helpers and copy/eigenvectors/inv/pinv/submatrix/transpose
...
share the ten matrix helpers and copy/eigenvectors/inv/pinv/submatrix/transpose continuation. Exact
...
template results additionally expose `.eigenvectors()`, `.inv()`, and `.pinv()`. Map templates such as
...
numeric `.eigenvectors()`, `.inv()`, and `.pinv()` added after the receiver-kind check and
copy/eigenvectors/inv/pinv/submatrix/transpose continuation; they do not widen imported UDT
identity.
