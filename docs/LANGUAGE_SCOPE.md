...
- user-defined type forms outside the local scalar-field subset; Phase J
  ...
  constructor returns, directly or through nested pure constructor-helper UDF
  calls, from local UDT parameter scalar fields, scalar fields read through
  ...
  only; field mutation inside functions or methods, `varip`, UDT history
  references, UDT fields, UDT arrays, and imported UDTs remain rejected
- user-defined method forms outside pure local-UDT receiver methods with scalar
  ...
  passthrough returns, and local UDT constructor returns, directly or through
  nested pure constructor-helper UDF calls, from receiver or local UDT
