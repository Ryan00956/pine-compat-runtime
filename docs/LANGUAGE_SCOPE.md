...
- `request.security("SYMBOL", timeframe, expression)` and
  ...
  `math.round_to_mintick`, `math.sum`, `ta.sma`, and `ta.ema`; local variable
  aliases and stateful math calls such as `math.random` are not part of this
  subset. Requested-context rolling callsite state is isolated from chart state.
