...
- user-defined type forms outside the local scalar-field subset; Phase J
  ...
  constructor returns from inferred scalar parameters using positional or named
  constructor field arguments only; field mutation inside functions or methods,
...
- user-defined method forms outside pure local-UDT receiver methods with scalar
  ...
  alias passthrough returns, nested-method UDT parameter passthrough returns,
  and local UDT constructor returns from receiver scalar fields or inferred
  scalar parameters using positional or named constructor field arguments; side
