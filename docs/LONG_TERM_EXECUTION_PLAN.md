...
- Local scalar-field user-defined types with `Type.new(...)`, field reads,
  ...
  UDF/method bodies, and UDF constructor returns, directly or through nested
  pure constructor-helper UDF calls, from local UDT parameter scalar fields,
...
- Pure methods on local UDT receivers with scalar or local UDT parameters and
  ...
  returns, directly or through nested pure constructor-helper UDF calls, from
  receiver or local UDT parameter scalar fields, scalar fields read through
  block-local receiver or local UDT parameter aliases, block-local scalar
  aliases of those fields, inferred scalar parameters, or block-local scalar
  aliases of those parameters using positional or named constructor field
  arguments.
