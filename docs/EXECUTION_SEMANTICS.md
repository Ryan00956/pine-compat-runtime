...
`na`, same-UDT constructors, or same-UDT `if` expressions. Local scalar-field
reassignment evaluates the right-hand expression, replaces that field in the
current UDT value, and writes the updated value back to the receiver symbol,
including the receiver's persistent slot when applicable. Local `for`
expressions may construct a local UDT in
