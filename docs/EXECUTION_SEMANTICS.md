...
`ta.sma`, `ta.ema`, `ta.rsi`, `ta.atr`, `ta.highest`, and `ta.lowest`. Rolling
callsite state for `math.sum` and `ta.*` calls is owned by the isolated
requested context. Stateful math calls such as `math.random` and extrema offset
helpers such as `ta.highestbars`/`ta.lowestbars` remain outside the
requested-expression subset.
