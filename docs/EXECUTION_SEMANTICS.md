...
share the twenty-seven matrix helpers and the copy/inv/submatrix/transpose/row/column/eigenvalue/
...
a concrete supported matrix kind now share the twenty-seven matrix helpers through
...
copy, and copy/inv/submatrix/transpose-continuation paths. Registered imported pure-function
...
independent-copy, and copy/inv/submatrix/transpose-continuation paths. Unknown/`na`, scalar, array,
...
below; producer-specific “copy/inv/submatrix/transpose-only” wording refers only to continuing as a
...
for numeric results, preserve shape and element kind, and keep source/nested-
...
results now share those direct helpers, add `.inv()` for numeric results, and
retain copy/inv/submatrix/transpose continuation while other
...
`values.submatrix(...)` results now share those helpers, add `.inv()` for
numeric results, and retain copy/inv/submatrix/transpose
...
and zero-dimension behavior, and shares the same eight helpers. Exact bound
numeric-matrix-receiver `values.kron(other)` results now share those helpers
and copy/inv/submatrix/transpose continuation while preserving the existing numeric input checks.
Exact namespace
...
`values.diff(other)` results now share the eight direct helpers and copy/inv/submatrix/transpose
...
results now share the eight direct helpers and copy/inv/submatrix/transpose continuation while
...
numeric-square-matrix-receiver `values.inv()` results now share the eight direct
helpers and copy/inv/submatrix/transpose continuation while preserving the existing receiver,
...
bound numeric-matrix-receiver `values.pinv()` results now share the eight direct
helpers and copy/inv/submatrix/transpose continuation while preserving the existing receiver,
...
`values.eigenvectors()` results now share the eight direct helpers and copy/inv/submatrix/transpose
...
expose `.inv()`. Exact supported scalar `map.new<K,V>` templates use the
