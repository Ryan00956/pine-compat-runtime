...
share the twenty-nine matrix helpers and the copy/eigenvectors/inv/pinv/submatrix/transpose/row/column/eigenvalue/
...
a concrete supported matrix kind now share the twenty-nine matrix helpers through
...
copy, and copy/eigenvectors/inv/pinv/submatrix/transpose-continuation paths. Registered imported pure-function
...
independent-copy, and copy/eigenvectors/inv/pinv/submatrix/transpose-continuation paths. Unknown/`na`, scalar, array,
...
below; producer-specific “copy/eigenvectors/inv/pinv/submatrix/transpose-only” wording refers only to continuing as a
...
`.eigenvectors()`, `.inv()`, and `.pinv()` for numeric results, preserve shape
...
results now share those direct helpers, add `.eigenvectors()`, `.inv()`, and
`.pinv()` for numeric results, and
retain copy/eigenvectors/inv/pinv/submatrix/transpose continuation while other
...
`.inv()`, and `.pinv()` for numeric results, and retain
...
and zero-dimension behavior, and shares the same ten helpers. Exact bound
numeric-matrix-receiver `values.kron(other)` results now share those helpers
and copy/eigenvectors/inv/pinv/submatrix/transpose continuation while preserving the existing numeric input checks.
Exact namespace
...
`values.diff(other)` results now share the ten direct helpers and copy/eigenvectors/inv/pinv/submatrix/transpose
...
results now share the ten direct helpers and copy/eigenvectors/inv/pinv/submatrix/transpose continuation while
...
numeric-square-matrix-receiver `values.inv()` results now share the ten direct
helpers and copy/eigenvectors/inv/pinv/submatrix/transpose continuation while preserving the existing receiver,
...
bound numeric-matrix-receiver `values.pinv()` results now share the ten direct
helpers and copy/eigenvectors/inv/pinv/submatrix/transpose continuation while preserving the existing receiver,
...
`values.eigenvectors()` results now share the ten direct helpers and copy/eigenvectors/inv/pinv/submatrix/transpose
...
expose `.eigenvectors()`, `.inv()`, and `.pinv()`. Exact supported scalar `map.new<K,V>` templates use the
