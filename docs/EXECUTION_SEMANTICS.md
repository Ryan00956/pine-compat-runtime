...
dimensions, optional `table.new` background-color, frame-color, frame-width, and
border-color initialization, and sparse cell snapshots for
...
`frame_color`, `frame_width`, and `border_color` initialize the table's final
background-color, frame-color, frame-width, and border-color values; optional
border-width initialization remains unsupported.
