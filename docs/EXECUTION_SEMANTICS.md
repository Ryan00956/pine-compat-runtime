...
share the twenty-six matrix helpers and the copy/submatrix/transpose/row/column/eigenvalue/
...
a concrete supported matrix kind now share the twenty-six matrix helpers through
...
copy, and copy/submatrix/transpose-continuation paths. Registered imported pure-function
...
independent-copy, and copy/submatrix/transpose-continuation paths. Unknown/`na`, scalar, array,
...
below; producer-specific “copy/submatrix/transpose-only” wording refers only to continuing as a
...
`values.copy()` results now share the same seven-helper contract, preserve shape
...
shares that six-helper and element-kind contract, swaps row/column shape, and
returns independent storage. Exact bound matrix-receiver `values.transpose()`
results now share those direct helpers and copy/submatrix/transpose continuation while other
matrix-valued helpers and mutation remain gated. Exact namespace
`matrix.submatrix(values, ...)` also shares the
six-helper and element-kind contract, returning an independent half-open range
with default full bounds and empty row/column slices. Exact bound matrix-receiver
`values.submatrix(...)` results now share those helpers and copy/submatrix/transpose
...
and zero-dimension behavior, and shares only the same seven helpers. Exact bound
numeric-matrix-receiver `values.kron(other)` results now share those helpers
and copy/submatrix/transpose continuation while preserving the existing numeric input checks.
Exact namespace
...
`values.diff(other)` results now share the seven direct helpers and copy/submatrix/transpose
...
results now share the seven direct helpers and copy/submatrix/transpose continuation while
...
numeric-square-matrix-receiver `values.inv()` results now share the seven direct
helpers and copy/submatrix/transpose continuation while preserving the existing receiver,
...
bound numeric-matrix-receiver `values.pinv()` results now share the seven direct
helpers and copy/submatrix/transpose continuation while preserving the existing receiver,
...
`values.eigenvectors()` results now share the seven direct helpers and copy/submatrix/transpose
...
the same seven helpers. Exact supported scalar `map.new<K,V>` templates use the
