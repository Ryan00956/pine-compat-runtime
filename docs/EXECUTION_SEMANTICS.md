...
`strategy.closedtrades.exit_bar_index(trade_num)`. They read the current
...
open-trade records. Stage 7 Slice 0 adds only script-visible closed-trade
`entry_price`, `exit_price`, `entry_bar_index`, and `exit_bar_index` namespace
functions; it does not add public trade-namespace fields. The prior Phase L boundary is
summarized in
