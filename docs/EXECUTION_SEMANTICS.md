...
terminal `.min(nth?)`, plus non-mutating `.abs()` transformation chains that
...
`.abs()`/`.min(nth?)`, are admitted after it. The lexical prefix
...
`.binary_search_leftmost()`/`.binary_search_rightmost()`/`.min()` results cannot continue
...
postfix helpers outside the thirteen-item read/copy/search/transform/aggregate set, and postfix
...
Numeric-only `.min(nth?)` returns the same `series int` or `series float` kind
as the receiver element type. After filtering `na` elements, `nth` is a zero-
based rank in ascending order and defaults to `0`; duplicate values retain
separate ranks. Dynamic integer ranks are accepted. Empty, all-`na`, and
...
plus `.binary_search_rightmost(value)`, `.abs()`, and `.min(nth?)`, where array `.copy()` and
numeric `.abs()` continue
...
and terminal `.min(nth?)`, with copy/abs array continuation. Mutation and
...
terminal `.min(nth?)` with ordinary numeric-array rank rules. Only
...
read/copy/search helpers plus numeric `.abs()` and terminal `.min(nth?)`, with
