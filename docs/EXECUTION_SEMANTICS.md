...
broader helpers, and mutation other than `.set(...)`/`.fill(...)`/`.reverse()`/`.reshape(...)`/`.swap_rows(...)` fail closed. The existing bound-receiver
...
share the thirty-eight matrix helpers and the copy/diff/eigenvectors/inv/kron/mult/pinv/pow/submatrix/transpose/row/column/eigenvalue/
...
a concrete supported matrix kind now share the thirty-eight matrix helpers through
...
mutation other than terminal `.set(...)`/`.fill(...)`/`.reverse()`/`.reshape(...)`/`.swap_rows(...)`, and terminal-read continuation cases
