...
share the thirty matrix helpers and the copy/eigenvectors/inv/pinv/pow/submatrix/transpose/row/column/eigenvalue/
...
a concrete supported matrix kind now share the thirty matrix helpers through
...
copy, and copy/eigenvectors/inv/pinv/pow/submatrix/transpose-continuation paths. Registered imported pure-function
...
independent-copy, and copy/eigenvectors/inv/pinv/pow/submatrix/transpose-continuation paths. Unknown/`na`, scalar, array,
...
below; producer-specific “copy/eigenvectors/inv/pinv/pow/submatrix/transpose-only” wording refers only to continuing as a
...
`.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)` for numeric results, preserve shape
...
`.pinv()`, and `.pow(power)` for numeric results, and
retain copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation while other
...
`.inv()`, `.pinv()`, and `.pow(power)` for numeric results, and retain
...
and zero-dimension behavior, and shares the same eleven helpers. Exact bound
numeric-matrix-receiver `values.kron(other)` results now share those helpers
and copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation while preserving the existing numeric input checks.
Exact namespace
...
`values.diff(other)` results now share the eleven direct helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose
...
results now share the eleven direct helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation while
...
numeric-square-matrix-receiver `values.inv()` results now share the eleven direct
helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation while preserving the existing receiver,
...
bound numeric-matrix-receiver `values.pinv()` results now share the eleven direct
helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation while preserving the existing receiver,
...
`values.eigenvectors()` results now share the eleven direct helpers and copy/eigenvectors/inv/pinv/pow/submatrix/transpose
...
expose `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)`. Exact supported scalar `map.new<K,V>` templates use the
