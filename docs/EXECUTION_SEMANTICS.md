...
`array.new_float`, `array.new_int`, `array.new_bool`, or `array.new_string`
declaration allocates a fresh array each time it executes. A `var` array
declaration keeps the same id and backing storage across bars, so mutations
such as `array.push` or `values.push(...)` persist.
