...
share the thirty-one matrix helpers and the copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose/row/column/eigenvalue/
...
a concrete supported matrix kind now share the thirty-one matrix helpers through
...
copy, and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose-continuation paths. Registered imported pure-function
...
independent-copy, and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose-continuation paths. Unknown/`na`, scalar, array,
...
below; producer-specific “copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose-only” wording refers only to continuing as a
...
`.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` for numeric results, preserve shape
...
results now share those direct helpers, add `.eigenvectors()`, `.inv()`,
`.kron(other)`, `.pinv()`, and `.pow(power)` for numeric results, and
retain copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation while other
...
`values.submatrix(...)` results now share those helpers, add `.eigenvectors()`,
`.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` for numeric results, and retain
...
and zero-dimension behavior, and shares the same twelve helpers. Exact bound
numeric-matrix-receiver `values.kron(other)` results now share those helpers
and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation while preserving the existing numeric input checks.
Exact namespace
...
`values.diff(other)` results now share the twelve direct helpers and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose
...
results now share the twelve direct helpers and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation while
...
numeric-square-matrix-receiver `values.inv()` results now share the twelve direct
helpers and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation while preserving the existing receiver,
...
bound numeric-matrix-receiver `values.pinv()` results now share the twelve direct
helpers and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation while preserving the existing receiver,
...
`values.eigenvectors()` results now share the twelve direct helpers and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose
...
expose `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)`. Exact supported scalar `map.new<K,V>` templates use the
...
admit direct binding and the closed `.size()`/`.get()`/`.first()`/`.last()`/
`.copy()` array-reader set, with
copy-only array continuation. Mutation and unsupported map templates remain
gated. Exact namespace
