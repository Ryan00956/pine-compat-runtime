...
share the twenty-eight matrix helpers and the copy/inv/pinv/submatrix/transpose/row/column/eigenvalue/
...
a concrete supported matrix kind now share the twenty-eight matrix helpers through
...
copy, and copy/inv/pinv/submatrix/transpose-continuation paths. Registered imported pure-function
...
independent-copy, and copy/inv/pinv/submatrix/transpose-continuation paths. Unknown/`na`, scalar, array,
...
below; producer-specific “copy/inv/pinv/submatrix/transpose-only” wording refers only to continuing as a
...
`values.copy()` results now share the same seven all-kind helpers, add `.inv()`
and `.pinv()` for numeric results, preserve shape and element kind, and keep source/nested-
copy storage independent; non-matrix receivers fail the existing method type
check. Exact namespace `matrix.transpose(values)` shares that seven-helper and
element-kind contract, swaps row/column shape, and
returns independent storage. Exact bound matrix-receiver `values.transpose()`
results now share those direct helpers, add `.inv()` and `.pinv()` for numeric results, and
retain copy/inv/pinv/submatrix/transpose continuation while other
...
`values.submatrix(...)` results now share those helpers, add `.inv()` and `.pinv()` for
numeric results, and retain copy/inv/pinv/submatrix/transpose
...
and zero-dimension behavior, and shares the same nine helpers. Exact bound
numeric-matrix-receiver `values.kron(other)` results now share those helpers
and copy/inv/pinv/submatrix/transpose continuation while preserving the existing numeric input checks.
Exact namespace
...
`values.diff(other)` results now share the nine direct helpers and copy/inv/pinv/submatrix/transpose
...
results now share the nine direct helpers and copy/inv/pinv/submatrix/transpose continuation while
...
numeric-square-matrix-receiver `values.inv()` results now share the nine direct
helpers and copy/inv/pinv/submatrix/transpose continuation while preserving the existing receiver,
...
bound numeric-matrix-receiver `values.pinv()` results now share the nine direct
helpers and copy/inv/pinv/submatrix/transpose continuation while preserving the existing receiver,
...
`values.eigenvectors()` results now share the nine direct helpers and copy/inv/pinv/submatrix/transpose
...
expose `.inv()` and `.pinv()`. Exact supported scalar `map.new<K,V>` templates use the
