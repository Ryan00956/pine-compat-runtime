...
subset. Stage 7 Slice 20 accepts finite non-negative integer const
`backtest_fill_limits_assumption` ticks for supported limit-order
verification. Percent-of-equity, cash sizing, contracts, other commission modes,
...
strategy             partial      declaration plus strategy-mode runtime result; positive const numeric initial_capital, fixed default_qty subset, supported cash commission modes, finite non-negative integer slippage ticks, and finite non-negative integer limit-verification ticks only
