...
produce float collection results. Matrix `.copy()` continues on the matrix-result path;
...
`na` propagates, and the reader is terminal. Other terminal readers, invalid
...
matrix kind through `SameAsArg`, and admit the same sixteen matrix helpers subject
