...
`.first()`, `.last()`, `.copy()`, and `.includes(value)` may follow one of
those producer calls;
...
array read/copy; the five terminal readers cannot continue into a user method
...
six postfix helpers: `.size()`, `.get(index)`, `.first()`, `.last()`, `.copy()`,
and `.includes(value)`. Only `.copy()` may continue into another allowed
read/copy; the other five results are terminal. `str.split` produces
`array<string>` and
...
use `.size()`, `.get(index)`, `.first()`, `.last()`, `.copy()`, and
`.includes(value)`.
...
to `.size()`/`.get()`/`.first()`/`.last()`/`.copy()`/`.includes(value)` with
copy-only array continuation and terminal membership checks. `.is_square()`
