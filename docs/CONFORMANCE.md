...
`.transpose()`. Numeric results additionally admit `.diff(other)`, `.eigenvectors()`, `.inv()`, `.kron(other)`, `.mult(other)`, `.pinv()`, and `.pow(power)`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same thirty-three-helper closed set through `$call_result`, subject to
...
allow `.copy()` plus numeric `.abs()` and `.standardize()`, and numeric-or-
...
The other readers are terminal or transition to the closed array-result path
...
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`/`.standardize()`/`.variance(biased?)`/`.stdev(biased?)`, plus int/float/string `.sort_indices(order?)`, may follow one of those producer calls;
...
UDT identity before lowering. Only `.copy()`, numeric `.abs()` and
`.standardize()`, and numeric-or-string `.sort_indices(order?)` may continue
with another allowed array chain; the twenty-three
terminal readers cannot continue
...
order or arity, direct mutation, and UDT call-result ordering before an
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same twenty-seven
...
`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`/`.standardize()`/`.variance(biased?)`/`.stdev(biased?)`, plus int/float/string `.sort_indices(order?)`. Only `.copy()`, numeric `.abs()` and
`.standardize()`, and numeric-or-string `.sort_indices(order?)` may continue
into another allowed array chain; the other
twenty-three results are
terminal.
`str.split` produces
`array<string>` and
...
`.binary_search(value)`/`.binary_search_leftmost(value)`/
`.binary_search_rightmost(value)`/`.abs()`/`.min(nth?)`/`.max(nth?)`/`.sum()`/
`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`/`.standardize()`/`.variance(biased?)`/`.stdev(biased?)`/`.sort_indices(order?)`.
...
`.binary_search_leftmost(value)`/`.binary_search_rightmost(value)`/`.abs()`/
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`/`.variance(biased?)`/`.stdev(biased?)` terminal reads plus transforming `.standardize()` and `.sort_indices(order?)`, with copy/abs/standardize/sort_indices array continuation and terminal read/search/aggregate checks. `.is_square()`
...
broader postfix helpers, and mutation fail closed.
...
matrix kind through `SameAsArg`, and admit the same thirty-three matrix helpers subject
...
initial/default-`na` cells, and fresh allocation. Only `.copy()` plus numeric
`.abs()` and `.standardize()` continue an array-result chain; matrix results
...
receivers instead enter the same thirty-three-helper closed set only for
...
function matrix results, broader-helper, mutation, and terminal-read
continuation cases remain gated.
