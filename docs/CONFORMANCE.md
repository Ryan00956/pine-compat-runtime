...
`strategy.closedtrades.profit(trade_num)` under the same contract. `trade_num`
is zero-based and integer-only; no matching trade, a negative index, an
out-of-range index, or a non-integer argument returns `na`. Public open-trade
records, open-trade namespace functions, closed-trade namespace functions
...
strategy.closedtrades.* partial   closed-trade entry_price, exit_price, entry_bar_index, exit_bar_index, size, and profit field functions in strategy-mode scripts only; trade_num is zero-based integer-only and invalid, negative, non-integer, or out-of-range indexes return na; no public runtime schema expansion
