...
allowlisted built-in array-producing receiver through `$builtin_array_result`.
...
`array.*` producer allowlist and the exact seven-producer cross-namespace set,
...
One later closed slice admits exactly seven non-`array` namespace producers on
that same `$builtin_array_result` path: `str.split`,
...
of the producer snapshot. The exact set does not include namespace-qualified
`matrix.mult(...)` as a direct-result receiver, even for an array-returning
overload; the existing bound-receiver `matrix_id.mult(array).size()` path is
unchanged. Matrix-returning calls, `map.new`/`matrix.new` templates, any other
namespace or non-producer member, and postfix mutation also stay outside this
set. The built-in lexical prefixes `str`, `ta`, `matrix`, and `map` stay
...
and the two exact built-in array-producing allowlists in the Built-In Runtime
Contract may be consumed directly through `.size()`, `.get(index)`, `.first()`,
...
exact allowlisted member or supported `array.new<T>` template, or one of the
seven exact non-`array` qualified producer names. Built-in-qualified/template
call results outside those two allowlists remain parser boundaries. For UDT
...
set, built-in-qualified/template call-result receivers outside the two exact
producer allowlists, and mutation through unsupported UDF/method side-effect
contexts remain outside the supported matrix. In particular, postfix reads do
