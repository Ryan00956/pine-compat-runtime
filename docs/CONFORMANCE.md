...
`var` persistence, and direct UDF parameter passthrough/returns. UDT values are
immutable in this subset. Field mutation, UDF construction from untyped scalar
...
user-defined types   partial      local scalar-field type declarations, Type.new constructors, field reads, ordinary variables, var persistence, and direct UDF parameter passthrough/returns only
