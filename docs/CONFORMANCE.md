...
`.transpose()`, plus terminal `.set(row, column, value)`, `.fill(value)`, `.reverse()`, `.reshape(rows, columns)`, and `.swap_rows(row1, row2)`. Numeric results
...
that same thirty-eight-helper closed set through `$call_result`, subject to
...
broader postfix helpers, and mutation other than `.set(...)`/`.fill(...)`/`.reverse()`/`.reshape(...)`/`.swap_rows(...)` fail closed.
...
matrix kind through `SameAsArg`, and admit the same thirty-eight matrix helpers subject
...
receivers instead enter the same thirty-eight-helper closed set only for
...
`.set(row, column, value)`/`.fill(value)`/`.reverse()`/`.reshape(rows, columns)`/`.swap_rows(row1, row2)`, and continuation after terminal reads remain gated.
