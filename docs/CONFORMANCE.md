...
built-in array producers through `$builtin_array_result`, and namespace
`matrix.copy(...)`/`matrix.transpose(...)`/`matrix.submatrix(...)`/
`matrix.kron(...)`/`matrix.diff(...)`/`matrix.pow(...)`/`matrix.inv(...)`/
`matrix.pinv(...)`/`matrix.mult(...)` candidates through
`$builtin_matrix_result`. Local UDF,
...
`matrix.kron(...)`, `matrix.diff(...)`, `matrix.pow(...)`, and `matrix.inv(...)`
plus `matrix.pinv(...)` results: `.rows()`, `.columns()`,
...
while admitting only the same five helpers. The existing bound-receiver
...
`matrix_id.pinv()` results remain gated. Other
matrix-returning calls, `map.new`/`matrix.new` templates, any other namespace
...
seven exact non-`array` qualified producer names. Namespace `matrix.mult`,
`matrix.copy`, `matrix.transpose`, `matrix.submatrix`, `matrix.kron`,
`matrix.diff`, `matrix.pow`, `matrix.inv`, and `matrix.pinv` use the separate
