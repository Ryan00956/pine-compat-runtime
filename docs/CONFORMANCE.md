...
floating profit for that same supported open position. `trade_num` is
...
records, open-trade namespace functions outside `entry_price`,
`entry_bar_index`, `entry_time`, `size`, and `profit`, closed-trade namespace
functions outside this subset, rich reporting metrics, and public output schema
...
strategy.opentrades.* partial     open-trade field function subset limited to entry_price, entry_bar_index, entry_time, size, and profit for the current supported long position; trade_num must be 0 and invalid or flat-state reads return na; no public runtime schema expansion
