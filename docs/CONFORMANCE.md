...
`matrix.copy(...)`/`matrix.mult(...)` candidates through
...
only `.rows()`, `.columns()`, `.elements_count()`, `.get(row, column)`, and
`.copy()`; `matrix.copy` preserves the source's supported matrix element kind.
Local-UDF scalar UDT
results may also use existing pure user methods when they carry a concrete
...
`matrix.copy` path remain rejected. So do non-array/non-matrix/non-UDT results,
unknown/`na` results without a concrete supported type or identity, mixed or
...
same copy-only continuation rule. The existing bound-receiver
`matrix_id.mult(array).size()` path is unchanged, while direct helpers on bound
matrix-returning `matrix_id.mult(...)` results and `matrix_id.copy()` results
remain gated. Other matrix-returning calls, `map.new`/`matrix.new` templates,
any other namespace or non-producer member, and postfix mutation also stay
outside this set. The
built-in lexical prefixes
...
seven exact non-`array` qualified producer names. Namespace `matrix.mult` and
`matrix.copy` use the separate `$builtin_matrix_result` prefix; semantic
...
the source's supported element kind. Only `.copy()` may continue on either
result kind, and bound or UDF matrix-result receivers do not enter this
namespace-only path.
