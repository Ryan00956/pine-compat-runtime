...
`.transpose()`, plus terminal `.set(row, column, value)`, `.fill(value)`, `.reverse()`, and `.reshape(rows, columns)`. Numeric results
...
that same thirty-seven-helper closed set through `$call_result`, subject to
...
broader postfix helpers, and mutation other than `.set(...)`/`.fill(...)`/`.reverse()`/`.reshape(...)` fail closed.
...
matrix kind through `SameAsArg`, and admit the same thirty-seven matrix helpers subject
...
receivers instead enter the same thirty-seven-helper closed set only for
...
`.set(row, column, value)`/`.fill(value)`/`.reverse()`/`.reshape(rows, columns)`, and continuation after terminal reads remain gated.
