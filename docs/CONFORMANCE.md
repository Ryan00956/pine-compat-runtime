...
`.is_square()`. Int inputs still produce float collection results. Matrix
...
non-finite results, propagates upstream `na`, and is terminal. Other terminal
readers, invalid arity or argument types, wrong-result helpers, broader postfix
helpers, and mutation fail closed.
...
matrix kind through `SameAsArg`, and admit the same seventeen matrix helpers subject
