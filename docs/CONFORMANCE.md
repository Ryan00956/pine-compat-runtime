...
continuation.
Admitted scalar map results expose `.size()`, terminal `.put(key, value)` and
`.clear()`, `.get(key)`, `.contains(key)`, `.copy()`, `.keys()`, and `.values()`, retain
...
template share those eight helpers through `$call_result`; parameter
...
other than terminal `.put(...)`/`.clear()`, array mutation outside the admitted derived-
...
admit `.size()`, terminal `.put(key, value)` and `.clear()`, `.get(key)`, `.contains(key)`,
`.copy()`, `.keys()`, and `.values()`. Only `.copy()` may continue another
admitted map helper; `.put(...)` and `.clear()` return `void` and cannot continue.
...
mutation other than terminal `.put(...)`/`.clear()`, unsupported templates, non-map copy
...
also share those eight helpers and copy-only continuation through `$call_result`;
...
isolation, clear-to-empty alias/fresh behavior, and UDF-side-effect rejection
