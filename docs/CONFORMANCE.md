...
`.transpose()`. Numeric results additionally admit `.eigenvectors()`, `.inv()`, and `.pinv()`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same twenty-nine-helper closed set through `$call_result`, subject to
...
admit `.eigenvectors()`, `.inv()`, and `.pinv()` with copy/eigenvectors/inv/pinv/submatrix/transpose
continuation.
...
matrix kind through `SameAsArg`, and admit the same twenty-nine matrix helpers subject
...
helpers plus `.eigenvectors()`, `.inv()`, and `.pinv()` for numeric results. Exact namespace `matrix.kron(left, right)` returns a fixed
...
same ten helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits those ten
...
same ten helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting the same ten helpers. Exact namespace
...
non-real, or incomplete results, while admitting the same ten helpers.
...
copy/eigenvectors/inv/pinv/submatrix/transpose continuation
...
and keep copy/eigenvectors/inv/pinv/submatrix/transpose continuation. Direct helpers on bound
...
also admit that set with copy/eigenvectors/inv/pinv/submatrix/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/eigenvectors/inv/pinv/submatrix/transpose-continuation paths. Remaining
...
same seven all-kind helpers plus `.eigenvectors()`, `.inv()`, and `.pinv()` for numeric results. Exact bound
...
receivers instead enter the same twenty-nine-helper closed set only for
...
copy/eigenvectors/inv/pinv/submatrix/transpose-continuation paths.
