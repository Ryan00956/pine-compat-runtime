...
`{{ticker}}`, `{{interval}}`, and `{{exchange}}`, in the `alertcondition`
message only.
...
alertcondition       partial      bool-compatible condition plus const-string title/message runtime events, with OHLCV plus ticker/interval/exchange placeholders in alertcondition messages only
