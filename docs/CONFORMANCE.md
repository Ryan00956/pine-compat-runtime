...
`.lastindexof(value)` may follow one of those producer calls;
...
array read/copy; the seven terminal readers cannot continue into a user method
...
eight postfix helpers: `.size()`, `.get(index)`, `.first()`, `.last()`,
`.copy()`, `.includes(value)`, `.indexof(value)`, and `.lastindexof(value)`.
Only `.copy()` may continue into another allowed read/copy; the other seven
...
terminal membership/first-and-last-index checks. `.is_square()`
