...
`.binary_search_leftmost(value)`/`.binary_search_rightmost(value)`, may follow
one of those producer calls;
...
array read/copy; the ten terminal readers cannot continue into a user method
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same eleven
...
`.binary_search_rightmost(value)`. Only `.copy()` may continue into another
allowed read/copy; the other ten results are terminal.
`str.split` produces
`array<string>` and
...
`.binary_search_leftmost(value)`/`.binary_search_rightmost(value)` with copy-
only array continuation and terminal membership/index checks. `.is_square()`
