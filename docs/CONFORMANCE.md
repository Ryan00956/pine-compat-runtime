...
resolve to a matrix can instead use only `.rows()`, `.columns()`,
`.elements_count()`, `.get(row, column)`, and `.copy()`. Local-UDF scalar UDT
...
and the result-type-checked namespace `matrix.mult` paths,
...
array or matrix helpers, and call-result mutation remain rejected.
...
postfix helpers fail closed. The existing bound-receiver
`matrix_id.mult(array).size()` path is unchanged, while direct helpers on bound
matrix-returning `matrix_id.mult(...)` results remain gated. Other
matrix-returning calls, `map.new`/`matrix.new` templates, any other namespace or
non-producer member, and postfix mutation also stay outside this set. The
...
seven exact non-`array` qualified producer names. Namespace `matrix.mult` uses
the separate `$builtin_matrix_result` prefix; semantic analysis admits the five
array helpers when its actual overload returns an array and the exact
`.rows()`/`.columns()`/`.elements_count()`/`.get(row, column)`/`.copy()` set
when it returns a matrix. Only `.copy()` may continue on either result kind,
