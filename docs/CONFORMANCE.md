...
`matrix.kron(...)`/`matrix.mult(...)` candidates through
...
the same set as namespace `matrix.transpose(...)`, `matrix.submatrix(...)`, and
`matrix.kron(...)` results: `.rows()`, `.columns()`, `.elements_count()`,
`.get(row, column)`, and `.copy()`.
...
`matrix.copy`/`matrix.transpose`/`matrix.submatrix`/`matrix.kron` paths remain
rejected. So do
...
set. The existing bound-receiver
...
`matrix_id.transpose()`/`matrix_id.submatrix()`/`matrix_id.kron(...)` results
remain gated. Other
...
`matrix.copy`, `matrix.transpose`, `matrix.submatrix`, and `matrix.kron` use the
separate `$builtin_matrix_result` prefix; semantic analysis admits the five
...
from its numeric operands. Only `.copy()` may continue on either result kind,
and bound or UDF matrix-result receivers do not enter this namespace-only path.
