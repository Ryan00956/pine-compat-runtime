...
that same seven-helper matrix set through `$call_result`; parameter passthrough,
...
`.copy()`, `.submatrix(...)`, and `.transpose()` may continue. Local and
imported user-method results with a concrete supported matrix kind share the
seven helpers through recorded method-call
...
matrix kind also share the seven helpers across alias-qualified, block/nested/
...
read/copy/submatrix/transpose helpers with copy/submatrix/transpose
continuation.
...
matrix kind through `SameAsArg`, and admit the same twenty-six matrix helpers subject
...
default full bounds and empty row/column slices, and shares only the same seven
...
the same seven helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits only those seven
...
the same seven helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting only the same seven helpers. Exact namespace
...
non-real, or incomplete results, while admitting only the same seven helpers.
...
cells, fresh allocation, and copy independence, and admit only the same seven
...
`.submatrix(...)`/`.transpose()` with copy/submatrix/transpose continuation
...
and keep copy/submatrix/transpose continuation. Direct helpers on bound
...
also admit that set with copy/submatrix/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/submatrix/transpose-continuation paths. Remaining
...
`.submatrix(...)`, or `.transpose()`. Exact bound matrix-receiver `values.copy()`
results preserve element kind, shape, and independent storage and admit the
same seven helpers. Exact bound matrix-receiver `values.transpose()` results
...
receivers instead enter the same seven-helper set only for unqualified local
...
copy/submatrix/transpose-continuation paths.
