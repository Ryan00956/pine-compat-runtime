...
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()`, may follow one of those producer calls;
...
with another allowed array chain; the fifteen terminal readers cannot continue
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same seventeen
...
`.binary_search_leftmost(value)`/`.binary_search_rightmost(value)`/`.abs()`/
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()` with copy/abs array continuation and terminal read/search/
