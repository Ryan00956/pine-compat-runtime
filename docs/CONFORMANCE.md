...
verification. Percent-of-equity, cash sizing, contracts, commission modes
outside the three listed above, richer fill models, and currency conversion remain
...
open trades. `strategy.equity` is cash plus current market value; without
configured commission this is equivalent to `initial_capital +
strategy.netprofit + strategy.openprofit` in the current subset, and with
supported commission it also reflects entry commission debits on open positions.
...
realized closed-trade losses as positive values.
...
strategy             partial      declaration plus strategy-mode runtime result; positive const numeric initial_capital, fixed default_qty subset, supported cash-per-contract, cash-per-order, and percent commission modes, finite non-negative integer slippage ticks, and finite non-negative integer limit-verification ticks only
