...
`strategy.closedtrades.exit_time(trade_num)`. `trade_num` is zero-based and
...
strategy.closedtrades.* partial   closed-trade entry_price, exit_price, entry_bar_index, exit_bar_index, entry_time, exit_time, size, and profit field functions in strategy-mode scripts only; trade_num is zero-based integer-only and invalid, negative, non-integer, or out-of-range indexes return na; no public runtime schema expansion
