...
`.is_identity()`, and `.is_symmetric()`, plus all-kind terminal `.is_square()`. Int inputs still produce
...
`na`, and is terminal. Other terminal readers, invalid
...
matrix kind through `SameAsArg`, and admit the same fourteen matrix helpers subject
