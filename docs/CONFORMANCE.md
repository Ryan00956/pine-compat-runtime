...
allow `.copy()` plus numeric `.abs()` and `.standardize()` to yield another
...
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`/`.standardize()`/`.variance(biased?)`/`.stdev(biased?)`, may follow one of those producer calls;
...
UDT identity before lowering. Only `.copy()`, numeric `.abs()`, and numeric
`.standardize()` may continue with another allowed array chain; the twenty-three
...
through another admitted reader, `.copy()`, `.abs()`, or `.standardize()`.
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same twenty-six
...
`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`/`.standardize()`/`.variance(biased?)`/`.stdev(biased?)`. Only `.copy()`, numeric `.abs()`, and
numeric `.standardize()` may continue into another allowed array chain; the other
...
`.binary_search(value)`/`.binary_search_leftmost(value)`/
`.binary_search_rightmost(value)`/`.abs()`/`.min(nth?)`/`.max(nth?)`/`.sum()`/
`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`/`.standardize()`/`.variance(biased?)`/`.stdev(biased?)`.
