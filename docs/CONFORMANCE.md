...
`.transpose()`. Numeric results additionally admit `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same thirty-one-helper closed set through `$call_result`, subject to
...
`.copy()`, numeric `.eigenvectors()`/`.inv()`/`.kron(other)`/`.pinv()`/`.pow(power)`,
...
admit `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` with copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose
continuation.
...
matrix kind through `SameAsArg`, and admit the same thirty-one matrix helpers subject
...
helpers plus `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` for numeric results. Exact namespace `matrix.kron(left, right)` returns a fixed
...
same twelve helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits those twelve
...
same twelve helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting the same twelve helpers. Exact namespace
...
non-real, or incomplete results, while admitting the same twelve helpers.
...
`.submatrix(...)`/`.transpose()`, plus numeric `.eigenvectors()`/`.inv()`/
`.pinv()`/`.pow(power)`, with
copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation
...
and keep copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation. Direct helpers on bound
...
copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose
...
also admit that set with copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose-continuation paths. Remaining
...
continue through `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, or `.pow(power)`.
Exact bound matrix-receiver `values.copy()`
results preserve element kind, shape, and independent storage and admit the
same seven all-kind helpers plus `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` for numeric results. Exact bound
...
receivers instead enter the same thirty-one-helper closed set only for
...
copy/eigenvectors/inv/kron/pinv/pow/submatrix/transpose-continuation paths.
