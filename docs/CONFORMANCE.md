...
`array<chart.point>` declarations, are fixture-backed with compatible or `na`
initializers and later compatible reassignment. Bare `array`, other non-scalar
...
typed declarations   partial      int, float, bool, string, color, chart.point, drawing-id label/line/linefill/box/table/polyline, scalar array<int>/array<float>/array<bool>/array<string>/array<color>, and array<chart.point> typed variable declarations with compatible or na initializers and later compatible reassignment are fixture-backed; bare array, other non-scalar array element types, UDT, map, matrix, and other typed declarations remain unsupported unless covered by narrower feature rows
