...
supported long position's entry fill bar for `trade_num == 0`. `trade_num` is
...
records, open-trade namespace functions outside `entry_price` and
`entry_bar_index`, closed-trade namespace functions outside this subset, rich
reporting metrics, and public output schema changes remain out of scope.
...
strategy.opentrades.* partial     open-trade field function subset limited to entry_price and entry_bar_index for the current supported long position; trade_num must be 0 and invalid or flat-state reads return na; no public runtime schema expansion
