...
strategy.closedtrades.* partial   closed-trade entry_price, exit_price, entry_bar_index, and exit_bar_index field functions in strategy-mode scripts only; trade_num is zero-based integer-only and invalid, negative, non-integer, or out-of-range indexes return na; no public runtime schema expansion
