...
`.min(nth?)`, may follow one of those producer calls;
...
`.min(nth?)` returns the same series numeric kind as the receiver element type.
It ranks non-`na` elements in ascending order with a zero-based optional `nth`
that defaults to `0`; dynamic integer ranks are accepted. Empty, all-`na`, and
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same thirteen
...
`.binary_search_rightmost(value)`/`.abs()`/`.min(nth?)`. Only `.copy()` and
...
`.binary_search(value)`/`.binary_search_leftmost(value)`/
`.binary_search_rightmost(value)`/`.abs()`/`.min(nth?)`.
