...
`.get(row, column)`, `.copy()`, `.row(index)`, and `.col(index)`. Int inputs still produce
...
matrix kind through `SameAsArg`, and admit the same seven matrix helpers with the
same matrix-copy/row/column-array continuation rule. Exact namespace `matrix.transpose(values)`
