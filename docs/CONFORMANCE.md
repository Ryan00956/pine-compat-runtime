...
`matrix.mult(...)` candidates through `$builtin_matrix_result`. Local UDF,
...
the same set as namespace `matrix.transpose(...)` and `matrix.submatrix(...)`
...
`matrix.copy`/`matrix.transpose`/`matrix.submatrix` paths remain rejected. So do
...
`matrix_id.transpose()`/`matrix_id.submatrix()` results remain gated. Other
...
`matrix.copy`, `matrix.transpose`, and `matrix.submatrix` use the separate
...
branch and returns the selected range with that kind. Only `.copy()` may
continue on either result kind, and bound or UDF matrix-result receivers do not enter this
namespace-only path.
