...
matrix-returning `matrix_id.mult(...)`, `matrix_id.copy()`, and
`matrix_id.transpose()`/`matrix_id.submatrix()`/`matrix_id.kron(...)`/
...
continue on either result kind, and bound or UDF matrix-result receivers do not
enter this namespace-only path.
