...
historical and forming-bar rollback lifecycle subset. `polyline.new` rejects
creation past the fixed 100-polyline runtime limit, and named
`max_polylines_count` declarations are stored in HIR. Typed declarations,
drawing point overloads, general polyline arrays, and runtime use of
declaration-driven max-count/eviction parity remain outside the supported
subset; see `docs/PHASE_E_POLYLINE_GATE.md`.
...
chart.point          partial      chart.point.new/now/from_index/from_time/copy constructors, time/index/price field reads, top-level field mutation, and array.new<chart.point>/array.from chart-point array storage/read/mutation/search are fixture-backed; polyline.new consumes chart-point arrays through its dedicated partial row; typed declarations, drawing point overloads, general polyline arrays, and runtime use of declaration-driven polyline max-count/eviction parity remain unsupported
