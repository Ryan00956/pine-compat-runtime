...
strategy.close       partial      full long-position close or fixed-qty partial close of the matching current long entry id at current bar close; fixed qty must be finite and positive, clamps to the current matching position size, keeps remaining long position state open at the same average price, preserves the public strategy JSON shape without close order events, and cancels matching pending exits only when the close fully flattens the entry; configured slippage worsens the long close fill price; qty_percent, close metadata, partial strategy.close_all, and multi-entry close allocation remain unsupported
