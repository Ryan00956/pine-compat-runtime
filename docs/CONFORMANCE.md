...
`.eigenvalues()` and `.is_zero()`, plus all-kind terminal `.is_square()`. Int inputs still produce
...
terminal without a prefix transition. Other terminal readers, invalid arity or argument
...
matrix kind through `SameAsArg`, and admit the same ten matrix helpers subject
to the numeric eigenvalue and zero-predicate checks, with the same matrix-copy/
