...
The current pure-internal call-result slice normalizes an unqualified plain
local UDF call receiver through the parser-only `$call_result` prefix. Local
UDF results can use the read-only array postfix helpers `.size()`,
`.get(index)`, `.first()`, `.last()`, and `.copy()` for every currently
supported array kind, or existing pure user methods when the result carries a
concrete local or imported scalar-tree UDT identity. Qualified user-defined
UDF/method results use the same array helper set. Built-in-qualified receivers
such as `array.copy(values).first()` and template call receivers such as
`array.new<int>(...).size()` remain parser boundaries; non-array/non-UDT
results, unknown/`na` results without a concrete supported type or identity,
mixed or non-scalar UDT-array identities, other array helpers, and call-result
...
results may be consumed directly through `.size()`, `.get(index)`, `.first()`,
`.last()`, or `.copy()` when they return any currently supported array kind,
including nested copy/read chains. The parser represents the unqualified form
with the impossible synthetic prefix `$call_result`; only a plain lexical
callee receives that normalization, so built-in-qualified and template call
results remain parser boundaries. For UDT arrays, the result must carry a
concrete same-local or same-imported scalar-tree identity. `get` preserves that
element identity and the existing named-index, `na`, negative-index, and bounds
semantics; `size` and `last` preserve the existing empty/typed-`na` behavior.
...
set, built-in-qualified/template call-result receivers, and mutation through
unsupported UDF/method side-effect contexts remain outside the supported
matrix.
