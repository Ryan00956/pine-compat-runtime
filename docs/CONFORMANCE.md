...
`.transpose()`. Numeric results additionally admit `.inv()` and `.pinv()`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same twenty-eight-helper closed set through `$call_result`, subject to
...
`.copy()`, numeric `.inv()`/`.pinv()`, `.submatrix(...)`, and `.transpose()` may
continue. Local and
...
admit `.inv()` and `.pinv()` with copy/inv/pinv/submatrix/transpose
continuation.
...
matrix kind through `SameAsArg`, and admit the same twenty-eight matrix helpers subject
...
helpers plus `.inv()` and `.pinv()` for numeric results. Exact namespace `matrix.kron(left, right)` returns a fixed
...
same nine helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits those nine
...
same nine helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting the same nine helpers. Exact namespace
...
non-real, or incomplete results, while admitting the same nine helpers.
...
copy/inv/pinv/submatrix/transpose continuation
...
and keep copy/inv/pinv/submatrix/transpose continuation. Direct helpers on bound
...
also admit that set with copy/inv/pinv/submatrix/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/inv/pinv/submatrix/transpose-continuation paths. Remaining
...
continue through `.inv()` or `.pinv()`. Exact bound matrix-receiver `values.copy()`
results preserve element kind, shape, and independent storage and admit the
same seven all-kind helpers plus `.inv()` and `.pinv()` for numeric results. Exact bound
...
receivers instead enter the same twenty-eight-helper closed set only for
...
copy/inv/pinv/submatrix/transpose-continuation paths.
