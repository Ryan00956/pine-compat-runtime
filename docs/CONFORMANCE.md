...
`.eigenvalues()`, plus all-kind terminal `.is_square()`. Int inputs still produce
...
transition. Other terminal readers, invalid arity or argument
...
matrix kind through `SameAsArg`, and admit the same nine matrix helpers subject
to the numeric eigenvalue check, with the same matrix-copy/array-result and
terminal-square-reader continuation rules. Exact namespace `matrix.transpose(values)`
