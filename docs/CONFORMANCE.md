...
`.eigenvalues()`. Int inputs still produce
...
continuation. Other terminal readers, invalid arity or argument
...
matrix kind through `SameAsArg`, and admit the same eight matrix helpers subject
to the numeric eigenvalue check, with the same matrix-copy/array-result
continuation rule. Exact namespace `matrix.transpose(values)`
