...
local UDF receiver through the parser-only `$call_result` prefix and an exact
allowlisted built-in array-capable receiver through `$builtin_array_result`.
Local UDF, qualified user-defined UDF/method, and allowlisted built-in producer
results can use only `.size()`, `.get(index)`, `.first()`, `.last()`, and
`.copy()` for currently supported array kinds. Local-UDF scalar UDT results may
also use existing pure user methods when they carry a concrete local or
imported scalar-tree identity. On the built-in producer path, only `.copy()`
may yield another array receiver for a nested allowed read/copy; terminal
`size`/`get`/`first`/`last` results cannot continue into user methods or other
...
and the array-returning `matrix.mult` overloads,
non-array/non-UDT results, unknown/`na` results without a concrete supported
type or identity, mixed or non-scalar UDT-array identities, other array
helpers, and call-result mutation remain rejected.
...
of the producer snapshot. A following dynamic exception admits
namespace-qualified `matrix.mult(...)` only when `MatrixMult` resolves the
actual overload to `array<float>`: matrix-by-array, array-by-matrix, and
array-by-array results may use the same five helpers, with only `.copy()` able
to continue a chain. Matrix-by-matrix, matrix-by-scalar, and scalar-by-matrix
overloads resolve to `matrix<float>` and remain fail-closed for direct
call-result array helpers. The existing bound-receiver
`matrix_id.mult(array).size()` path is unchanged. Other matrix-returning calls,
`map.new`/`matrix.new` templates, any other namespace or non-producer member,
and postfix mutation also stay outside this set. The built-in lexical prefixes
`str`, `ta`, `matrix`, and `map` stay
reserved for this recognition and cannot
...
seven exact non-`array` qualified producer names, or `matrix.mult`. Semantic
analysis admits the last candidate only when its actual overload returns an
array; built-in-qualified/template call results outside these paths remain
parser boundaries. For UDT
...
static and array-capable paths, and mutation through unsupported UDF/method
...
array.*              partial      float/int/bool/string/color/color and label/line/linefill/polyline/box/table creation through type-specific array.new_* calls and official array.new<type> syntax, chart.point array.new<chart.point> construction, array.from inference, same-local scalar-tree UDT array.new<T>/array.from construction with typed array<T>/T[] declarations, same-imported scalar-tree UDT array.from and typed array<lib.Type>/lib.Type[] size/get/first/last, set replacement field reads, push append field reads, unshift prepend field reads, insert insertion field reads, fill replacement field reads, join positional stringification, includes/indexof/lastindexof structural equality search, sort/sort_indices by int/float/string sort_field, pop/remove/shift return field reads, clear size reset, copy independent field reads, reverse reordered field reads, slice window field reads, concat appended field reads, and statement/expression/index-value for-in value-copy field reads, size reads, array.get field reads, array.set replacement, array.push append, array.insert same-UDT insertion, array.pop returns, array.remove returns, array.shift returns, array.unshift same-UDT prepend, array.fill same-UDT replacement, array.join positional UDT stringification, array.first reads, array.last reads, array.clear reset/reuse, array.copy independence, array.concat same-UDT append, array.slice parent-window read/write mirroring, array.reverse reordering, same-local and same-imported UDT array.sort and array.sort_indices by int/float/string sort_field, same-local and same-imported scalar-tree UDT array includes/indexof/lastindexof structural equality search, same-local scalar-tree UDT array id history snapshots including dynamic na-offset predicate output, label-array, line-array, linefill-array, polyline-array, box-array, and table-array for-in shallow-id iteration, chart-point plus same-local and same-imported scalar-tree UDT array for-in value-copy iteration, array<int>, array<float>, array<bool>, array<string>, array<color>, array<label>, array<line>, array<linefill>, array<polyline>, array<box>, array<table>, array<chart.point>, and same-local or same-imported scalar-tree UDT arrays index/value for-in iteration with a zero-based series int index loop local, and ordinary var realtime rollback, reference, copy, get/set/insert/remove with negative indexes, fill, slice/concat, search including linefill, polyline, and chart.point object-array search, binary search, float/int/bool truth helpers, numeric abs/statistics/range/median/mode/percentile/covariance/standardize/variance/stdev, numeric/string sort and sort_indices, branch/loop sort and reverse mutation fixtures, scalar-array plus same-local and same-imported scalar-tree UDT-array join, mutation, polyline array/slice history snapshots, scalar-array while-expression result history snapshots including dynamic na-offset predicates, and helper fixture subset only; imported UDF and user-method same-imported scalar-tree UDT array returns are fixture-backed for direct/alias, copy/new/from, private nested, final-flow, typed named-argument, type-position rewrite, and same-library dual-alias isolation paths; local/imported tuple returns preserve identity independently per destructured UDT-array slot; qualified user-defined UDF/method and unqualified plain local UDF results returning supported array kinds, plus the exact array.* producer allowlist, seven fixed cross-namespace scalar-array producers, and array-returning matrix.mult(...) overloads documented above support direct size/get/first/last/copy chaining with concrete same-local/same-imported scalar-tree identity required for UDT arrays; built-in producer chains may continue only through copy, while size/get/first/last are terminal and cannot invoke user or other call-result methods; concrete scalar UDT local-UDF results may invoke existing pure user methods; conflicting scalar-return or tuple-slot identities, non-scalar UDT-array returns, non-array/non-UDT results or unknown/na results without a concrete supported type or identity, built-in-qualified/template call-result receivers outside the exact static and array-capable paths, other array helpers, postfix mutation, and concat producer use inside UDFs remain unsupported
typed declarations   partial      int, float, bool, string, color, chart.point including chart.point varip declarations, drawing-id label/line/linefill/box/table/polyline, scalar array<int>/array<float>/array<bool>/array<string>/array<color>, object-id array<label>/array<line>/array<linefill>/array<polyline>/array<box>/array<table>, array<chart.point>, same-local non-scalar typed-`na` UDT declarations for direct, `var`, ternary, `if`, `switch`, `for`, `for...in`, `while`, UDF passthrough, method parameter passthrough, and method receiver/nested receiver passthrough identity-only history checks, same-local scalar-tree UDT array<T>, same-imported scalar-tree UDT array<lib.Type>/lib.Type[] declarations, same-local or same-imported scalar-tree UDT array varip declarations, scalar-tree imported UDT declarations initialized or reassigned from the same imported identity, including ordinary var and scalar-tree varip declarations, and equivalent type[] aliases for those supported array element types including var declarations and the scalar typed-array varip subset, explicitly typed same-local scalar-tree UDT varip declarations initialized from na, same-UDT constructors, same-identity aliases, same-UDT ternary expressions, same-UDT switch expressions, same-UDT if expressions, same-UDT for expressions, same-UDT for...in expressions, or same-UDT while expressions, including nested scalar-tree Wrapper values initialized from those expression forms, matrix<float>, matrix<int>, matrix<bool>, matrix<string>, or matrix<color> declarations with compatible or na initializers and later compatible reassignment, and scalar map<K,V> declarations with compatible or na initializers and later same-template reassignment, and bare scalar map declarations initialized from known scalar map expressions are fixture-backed; bare array, non-scalar UDT arrays, template-less bare map declarations, map templates beyond int/float/bool/string/color keys and values, bare matrix, matrix templates beyond float/int/bool/string/color, cross-element matrix declaration initializers, and other typed declarations remain unsupported unless covered by narrower feature rows; typed same-imported scalar-tree UDT array destinations accept the fixture-backed imported UDF/method and unqualified root-local wrapper return paths, while cross-identity scalar returns, conflicting tuple-slot identities, non-scalar forms, built-in-qualified/template call-result receivers outside the exact built-in array producer and cross-namespace array-capable paths, terminal built-in producer reader results followed by user or other call-result methods, and array call-result helpers outside size/get/first/last/copy remain unsupported
