...
`.transpose()`. Numeric results additionally admit `.diff(other)`, `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same thirty-two-helper closed set through `$call_result`, subject to
...
`.copy()`, numeric `.diff(other)`/`.eigenvectors()`/`.inv()`/`.kron(other)`/`.pinv()`/`.pow(power)`,
...
admit `.diff(other)`, `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` with copy/diff/eigenvectors/inv/kron/pinv/pow/submatrix/transpose
continuation.
...
matrix kind through `SameAsArg`, and admit the same thirty-two matrix helpers subject
...
helpers plus `.diff(other)`, `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` for numeric results. Exact namespace `matrix.kron(left, right)` returns a fixed
...
same thirteen helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits those thirteen
...
same thirteen helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting the same thirteen helpers. Exact namespace
...
non-real, or incomplete results, while admitting the same thirteen helpers.
...
`.inv()`/`.kron(other)`/`.pinv()`/`.pow(power)`, with
copy/diff/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation
...
and keep copy/diff/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation. Direct helpers on bound
...
also admit that set with copy/diff/eigenvectors/inv/kron/pinv/pow/submatrix/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/diff/eigenvectors/inv/kron/pinv/pow/submatrix/transpose-continuation paths. Remaining
...
continue through `.diff(other)`, `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, or `.pow(power)`.
Exact bound matrix-receiver `values.copy()`
results preserve element kind, shape, and independent storage and admit the
same seven all-kind helpers plus `.diff(other)`, `.eigenvectors()`, `.inv()`, `.kron(other)`, `.pinv()`, and `.pow(power)` for numeric results. Exact bound
...
receivers instead enter the same thirty-two-helper closed set only for
...
copy/diff/eigenvectors/inv/kron/pinv/pow/submatrix/transpose-continuation paths.
