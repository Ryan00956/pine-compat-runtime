...
`.first()`, `.last()`, `.copy()`, `.includes(value)`, and `.indexof(value)` may
...
array read/copy; the six terminal readers cannot continue into a user method
...
seven postfix helpers: `.size()`, `.get(index)`, `.first()`, `.last()`,
`.copy()`, `.includes(value)`, and `.indexof(value)`. Only `.copy()` may
continue into another allowed read/copy; the other six results are terminal.
`str.split` produces
`array<string>` and
