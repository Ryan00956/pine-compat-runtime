...
`$builtin_matrix_result`. Exact supported scalar `map.new<K,V>` templates use
the separate `$builtin_map_result` prefix. Local UDF,
...
Map mutation, direct `keys()`/`values()`, unsupported templates, and other map
call-result receivers remain parser/semantic boundaries.
