...
`matrix.kron(...)`/`matrix.diff(...)`/`matrix.pow(...)`/`matrix.mult(...)`
candidates through
...
`matrix.kron(...)`, `matrix.diff(...)`, and `matrix.pow(...)` results:
`.rows()`, `.columns()`, `.elements_count()`, `.get(row, column)`, and `.copy()`.
...
`matrix.diff`/`matrix.pow` paths remain rejected. So do
...
helpers. The existing bound-receiver
...
`matrix_id.diff(...)`/`matrix_id.pow(...)` results remain gated. Other
...
`matrix.copy`, `matrix.transpose`, `matrix.submatrix`, `matrix.kron`,
`matrix.diff`, and `matrix.pow` use the separate `$builtin_matrix_result`
prefix; semantic
analysis admits the five
...
identity, copy, and positive powers. Only `.copy()` may
