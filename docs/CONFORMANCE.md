...
- unsupported strategy declaration contexts and strategy order functions such as
  `strategy.exit` and `strategy.order`; `strategy.exit` limit, profit, loss,
  trailing, partial quantity, combined stop/limit, and missing-entry forms
  remain fixture-backed unsupported cases until an OHLC fill policy and
  pending-order contract are designed. Stop-exit calls may be accepted
  semantically during Phase M staging but still emit a strategy runtime
  diagnostic until broker fills land.
...
strategy.exit       unsupported  semantic/runtime staging accepts stop-only calls and records broker-owned pending stops internally; executable fills remain unimplemented
strategy.*           unsupported  strategy order functions beyond strategy.entry/strategy.close, executable strategy.exit runtime fills, strategy.exit limit/combined/profit/loss/trailing/partial/missing-entry forms, rich order types, percent/cash/contracts sizing, mutable strategy state, and strategy reporting helpers beyond the supported position/profit/equity variables are not implemented
