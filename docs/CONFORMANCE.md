...
`.binary_search_leftmost(value)`, may follow one of those producer calls;
...
array read/copy; the nine terminal readers cannot continue into a user method
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same ten
parser helpers: `.size()`, `.get(index)`, `.first()`, `.last()`, `.copy()`,
`.includes(value)`, `.indexof(value)`, `.lastindexof(value)`, and numeric-only
`.binary_search(value)`/`.binary_search_leftmost(value)`. Only `.copy()` may
continue into another allowed read/copy; the other nine results are terminal.
`str.split` produces
`array<string>` and
...
`.binary_search_leftmost(value)` with copy-only array continuation and terminal
membership/index checks. `.is_square()`
