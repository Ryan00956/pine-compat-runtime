...
`matrix.mult(...)` candidates through
...
results: `.rows()`, `.columns()`, `.elements_count()`, `.get(row, column)`, and
`.copy()`.
...
the same five helpers. The existing bound-receiver
...
`matrix_id.diff(...)`/`matrix_id.pow(...)`/`matrix_id.inv()` results remain
gated. Other
...
`matrix.copy`, `matrix.transpose`, `matrix.submatrix`, `matrix.kron`,
`matrix.diff`, `matrix.pow`, and `matrix.inv` use the separate
...
for invertible inputs and yields `na` for singular or invalid-cell inputs. Only `.copy()` may
