...
`.copy()` may continue. Local and imported user-method results with a concrete
supported matrix kind share the five helpers through recorded method-call
...
matrix kind also share the five helpers across alias-qualified, block/nested/
...
`.get(row, column)`, `.copy()`, and `.row(index)`. Int inputs still produce
float collection results. Matrix `.copy()` continues on the matrix-result path;
`.row(index)` returns a fresh element-kind-preserving scalar array and switches
to `.size()`/`.get()`/`.first()`/`.last()`/`.copy()` with copy-only array
continuation. Other terminal readers, `.col()`, invalid arity or argument
types, wrong-result helpers, broader postfix helpers, and mutation fail closed.
...
matrix kind through `SameAsArg`, and admit the same six matrix helpers with the
same matrix-copy/row-array continuation rule. Exact namespace `matrix.transpose(values)`
...
block/nested/control-flow, and copy-only-continuation paths. Remaining
...
zero-dimension, dual-alias, independent-copy, and copy-only-continuation paths.
...
kind, zero-dimension, dual-alias, independent-copy, and copy-only-continuation
