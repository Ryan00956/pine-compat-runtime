...
scalar-tree UDT-array identity may be consumed directly through `.first()` or
`.copy()`, including `.copy().first()`; parser-synthetic postfix calls remain
distinct from explicit same-named imported functions. Conflicting identities
within one scalar return or tuple slot, non-scalar imported array returns,
same-local or other direct call-result array methods, and mutation through
