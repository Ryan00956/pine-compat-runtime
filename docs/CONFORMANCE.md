...
`matrix.kron(...)`/`matrix.diff(...)`/`matrix.mult(...)` candidates through
...
`matrix.kron(...)`, and `matrix.diff(...)` results: `.rows()`, `.columns()`,
...
`matrix.diff` paths remain rejected. So do
...
`matrix_id.diff(...)` results remain gated. Other
...
`matrix.copy`, `matrix.transpose`, `matrix.submatrix`, `matrix.kron`, and
`matrix.diff` use the separate `$builtin_matrix_result` prefix; semantic
