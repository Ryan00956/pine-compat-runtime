...
long position's entry price for `trade_num == 0`. `trade_num` is zero-based and
integer-only; no matching trade, a negative index, an out-of-range index, or a
non-integer argument returns `na`. Public open-trade records, open-trade
namespace functions outside `entry_price`, closed-trade namespace functions
...
strategy.opentrades.* partial     open-trade field function subset limited to entry_price for the current supported long position; trade_num must be 0 and invalid or flat-state reads return na; no public runtime schema expansion
