...
`.transpose()`. Numeric results additionally admit `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same thirty-helper closed set through `$call_result`, subject to
...
`.copy()`, numeric `.eigenvectors()`/`.inv()`/`.pinv()`/`.pow(power)`,
...
admit `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)` with copy/eigenvectors/inv/pinv/pow/submatrix/transpose
continuation.
...
matrix kind through `SameAsArg`, and admit the same thirty matrix helpers subject
...
helpers plus `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)` for numeric results. Exact namespace `matrix.kron(left, right)` returns a fixed
...
same eleven helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits those eleven
...
same eleven helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting the same eleven helpers. Exact namespace
...
non-real, or incomplete results, while admitting the same eleven helpers.
...
`.pinv()`/`.pow(power)`, with
copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation
...
and keep copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation. Direct helpers on bound
...
copy/eigenvectors/inv/pinv/pow/submatrix/transpose
...
also admit that set with copy/eigenvectors/inv/pinv/pow/submatrix/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/eigenvectors/inv/pinv/pow/submatrix/transpose-continuation paths. Remaining
...
continue through `.eigenvectors()`, `.inv()`, `.pinv()`, or `.pow(power)`.
Exact bound matrix-receiver `values.copy()`
results preserve element kind, shape, and independent storage and admit the
same seven all-kind helpers plus `.eigenvectors()`, `.inv()`, `.pinv()`, and `.pow(power)` for numeric results. Exact bound
...
receivers instead enter the same thirty-helper closed set only for
...
copy/eigenvectors/inv/pinv/pow/submatrix/transpose-continuation paths.
