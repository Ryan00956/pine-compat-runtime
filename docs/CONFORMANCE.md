...
`.eigenvalues()`, `.is_zero()`, `.is_binary()`, and `.is_diagonal()`, plus all-kind terminal `.is_square()`. Int inputs still produce
...
true, and upstream-`na`
propagation, and is terminal as well. Other terminal readers, invalid arity or
argument types, wrong-result helpers, broader postfix helpers, and mutation
fail closed.
...
matrix kind through `SameAsArg`, and admit the same twelve matrix helpers subject
