...
supported open position under the no-commission account model. `trade_num` is
zero-based and integer-only; no matching trade, a negative index, an
out-of-range index, or a non-integer argument returns `na`. Public open-trade
records, open-trade namespace functions outside `entry_price`, `entry_id`,
`entry_bar_index`, `entry_time`, `size`, `profit`, and `commission`,
closed-trade namespace functions outside this subset, rich reporting metrics,
and public output schema changes remain out of scope.
...
strategy.closedtrades.* partial   closed-trade entry_price, entry_id, exit_price, exit_id, entry_bar_index, exit_bar_index, entry_time, exit_time, commission, size, and profit field functions in strategy-mode scripts only; entry_id returns the retained entry id; exit_id returns the retained close or exit id; commission is 0.0 in the current no-commission account model; trade_num is zero-based integer-only and invalid, negative, non-integer, or out-of-range indexes return na; no public runtime schema expansion
...
strategy.opentrades.* partial     open-trade field function subset limited to entry_price, entry_id, entry_bar_index, entry_time, size, profit, and commission for the current supported long position; trade_num must be 0 and invalid or flat-state reads return na; commission returns 0.0 in the current no-commission account model; no public runtime schema expansion
