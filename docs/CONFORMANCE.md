...
`matrix.copy(...)`/`matrix.transpose(...)`/`matrix.mult(...)` candidates through
`$builtin_matrix_result`. Local UDF, qualified user-defined UDF/method, and
admitted built-in producer results can
...
the same set as namespace `matrix.transpose(...)` results: `.rows()`,
`.columns()`, `.elements_count()`, `.get(row, column)`, and `.copy()`.
`matrix.copy` preserves shape, while `matrix.transpose` swaps shape; both
preserve the source's supported matrix element kind.
Local-UDF scalar UDT results may also use existing pure user methods when they
carry a concrete
local or imported scalar-tree identity. On either built-in producer path, only
`.copy()` may yield another collection receiver for a nested allowed read/copy;
...
`matrix.copy`/`matrix.transpose` paths remain rejected. So do
non-array/non-matrix/non-UDT results, unknown/`na` results without a concrete
supported type or identity, mixed or
non-scalar UDT-array identities, other array or matrix helpers, and call-result
mutation.
...
storage. The existing bound-receiver
...
`matrix_id.transpose()` results remain gated. Other matrix-returning calls,
...
`matrix.copy`, and `matrix.transpose` use the separate
...
preserve the source's supported element kind. Only `.copy()` may continue on
either result kind, and bound or UDF matrix-result receivers do not enter this
namespace-only path.
