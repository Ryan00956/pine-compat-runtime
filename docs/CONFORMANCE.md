...
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()`/`.median()`, may follow one of those producer calls;
...
with another allowed array chain; the sixteen terminal readers cannot continue
...
`.avg()`/`.range()`/`.median()`. Only `.copy()` and
...
`.binary_search_leftmost(value)`/`.binary_search_rightmost(value)`/`.abs()`/
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()`/`.median()` with copy/abs array continuation and terminal read/search/
