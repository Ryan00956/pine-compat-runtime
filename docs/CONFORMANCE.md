...
`.transpose()`. Numeric results additionally admit `.inv()`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same twenty-seven-helper closed set through `$call_result`, subject to
...
`.copy()`, numeric `.inv()`, `.submatrix(...)`, and `.transpose()` may
continue. Local and
...
matrix kind through `SameAsArg`, and admit the same twenty-seven matrix helpers subject
...
helpers plus `.inv()` for numeric results. Exact namespace `matrix.kron(left, right)` returns a fixed
...
same eight helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits those eight
...
same eight helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting the same eight helpers. Exact namespace
...
non-real, or incomplete results, while admitting the same eight helpers.
...
seven helpers, while numeric templates additionally admit `.inv()`. Invalid
...
`.submatrix(...)`/`.transpose()`, plus numeric `.inv()`, with
copy/inv/submatrix/transpose continuation
...
and keep copy/inv/submatrix/transpose continuation. Direct helpers on bound
...
also admit that set with copy/inv/submatrix/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/inv/submatrix/transpose-continuation paths. Remaining
...
continue through `.inv()`. Exact bound matrix-receiver `values.copy()`
results preserve element kind, shape, and independent storage and admit the
same seven all-kind helpers plus `.inv()` for numeric results. Exact bound
...
receivers instead enter the same twenty-seven-helper closed set only for
...
copy/inv/submatrix/transpose-continuation paths.
