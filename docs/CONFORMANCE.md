...
supported long position's entry fill timestamp for `trade_num == 0`.
...
`entry_bar_index`, and `entry_time`, closed-trade namespace functions outside
this subset, rich reporting metrics, and public output schema changes remain
out of scope.
...
strategy.opentrades.* partial     open-trade field function subset limited to entry_price, entry_bar_index, and entry_time for the current supported long position; trade_num must be 0 and invalid or flat-state reads return na; no public runtime schema expansion
