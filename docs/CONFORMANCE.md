...
`box.new`, selected geometry/background/border/extend/text/text-color mutators,
`box.delete`, and fixture-backed cloning with `box.copy` over the latest existing box
...
snapshots. `box.set_text_color` records text color in box snapshots; text size,
style, and layout methods remain unsupported. `box.get_left`, `box.get_right`,
