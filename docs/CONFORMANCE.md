...
`.binary_search_leftmost(value)`/`.binary_search_rightmost(value)`/`.abs()`, may
follow one of those producer calls;
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same twelve
...
`.binary_search_rightmost(value)`/`.abs()`. Only `.copy()` and numeric `.abs()`
may continue into another allowed array chain; the other ten results are
terminal.
`str.split` produces
`array<string>` and
...
`.binary_search(value)`/`.binary_search_leftmost(value)`/
`.binary_search_rightmost(value)`/`.abs()`.
...
copy/abs array continuation and terminal membership/index checks. `.is_square()`
