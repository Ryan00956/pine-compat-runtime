...
`.lastindexof(value)`, plus numeric-only `.binary_search(value)`, may follow one
of those producer calls;
...
array read/copy; the eight terminal readers cannot continue into a user method
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same nine
parser helpers: `.size()`, `.get(index)`, `.first()`, `.last()`, `.copy()`,
`.includes(value)`, `.indexof(value)`, `.lastindexof(value)`, and numeric-only
`.binary_search(value)`. Only `.copy()` may continue into another allowed
read/copy; the other eight results are terminal.
`str.split` produces
`array<string>` and
...
`.indexof(value)`/`.lastindexof(value)`/`.binary_search(value)` with copy-only
array continuation and terminal membership/index checks. `.is_square()`
