...
`.is_stochastic()`, plus numeric-only terminal `.sum()`/`.avg()` and all-kind
terminal `.is_square()`. Int inputs still produce float collection results. Matrix
...
matrix kind through `SameAsArg`, and admit the same eighteen matrix helpers subject
