...
`.first()` or `.copy()`, including `.copy().first()`; parser-synthetic postfix
calls remain distinct from explicit same-named local methods or imported
functions. Conflicting identities within one scalar return or tuple slot,
non-scalar array returns, unqualified local UDF results, other direct
call-result array methods, and mutation through unsupported UDF/method
side-effect contexts remain outside the supported matrix.
