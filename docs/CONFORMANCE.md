...
horizontal-alignment mutators, `box.delete`, and fixture-backed cloning with
`box.copy` over the latest existing box
...
records horizontal text alignment in box snapshots; vertical alignment, wrap,
font, and richer layout methods remain unsupported. `box.get_left`,
`box.get_right`, `box.get_top`, and `box.get_bottom` read the latest existing box
snapshot and return `na` for `na` or deleted boxes; other box methods remain
unsupported.
