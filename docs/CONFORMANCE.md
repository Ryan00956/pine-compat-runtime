...
`.eigenvalues()`, `.is_zero()`, and `.is_binary()`, plus all-kind terminal `.is_square()`. Int inputs still produce
...
is terminal as well. Other terminal readers, invalid arity or argument
...
matrix kind through `SameAsArg`, and admit the same eleven matrix helpers subject
to the numeric eigenvalue, zero-predicate, and binary-predicate checks, with the same matrix-copy/
