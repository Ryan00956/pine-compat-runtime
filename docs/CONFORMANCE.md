...
carry a concrete local or imported scalar-tree identity. Array and map result
paths allow only `.copy()` to yield another same-family collection receiver;
...
`.min(nth?)`/`.max(nth?)`/`.sum()`/`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`, may follow one of those producer calls;
...
with another allowed array chain; the twenty-one terminal readers cannot continue
...
through another admitted reader, `.copy()`, or `.abs()`.
...
`matrix.eigenvalues`, `map.keys`, and `map.values`. They share the same eighteen
...
`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`. Only `.copy()` and
...
`.binary_search(value)`/`.binary_search_leftmost(value)`/
`.binary_search_rightmost(value)`/`.abs()`/`.min(nth?)`/`.max(nth?)`/`.sum()`/
`.avg()`/`.range()`/`.median()`/`.mode()`/`.percentile_nearest_rank(percentage)`/`.percentile_linear_interpolation(percentage)`/`.percentrank(index)`/`.covariance(id2, biased?)`.
...
initial/default-`na` cells, and fresh allocation. Only `.copy()` continues an
array-result chain; matrix results may continue through `.copy()`,
