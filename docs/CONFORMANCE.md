...
`.get(key)`, `.contains(key)`, `.copy()`, `.keys()`, and `.values()`, retain
...
user-method alias results update shared storage.
Unqualified local-UDF results that retain one concrete supported scalar map
template share those seven helpers through `$call_result`; parameter
...
scalar, array, matrix, wrong-template/key/value, broader-helper, map mutation
other than terminal `.put(...)`, array mutation outside the admitted derived-
...
admit `.size()`, terminal `.put(key, value)`, `.get(key)`, `.contains(key)`,
`.copy()`, `.keys()`, and `.values()`. Only `.copy()` may continue another
admitted map helper; `.put(...)` returns `void` and cannot continue.
...
mutation other than terminal `.put(...)`, unsupported templates, non-map copy
...
also share those seven helpers and copy-only continuation through `$call_result`;
...
isolation, and UDF-side-effect rejection retain ordinary `map.put` behavior.
