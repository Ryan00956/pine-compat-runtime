...
the result-type-checked namespace `matrix.mult` paths, and the exact namespace
`matrix.copy`/`matrix.transpose`/`matrix.submatrix`/`matrix.kron`/
`matrix.diff`/`matrix.pow`/`matrix.inv` paths remain rejected. So do
...
continuation. Direct helpers on bound `matrix_id.mult(...)`,
numeric `matrix_id.diff(other)` results admit the same helpers for matrix or
scalar operands, preserve operand direction and selected matrix shape, and
resolve to independent `matrix<float>` storage. Direct helpers on bound
`matrix_id.pow(...)`/`matrix_id.inv()`/
`matrix_id.pinv()`/`matrix_id.eigenvectors()` results remain gated. Other
matrix-returning calls, unsupported `map.new` and `matrix.new` templates, any other namespace
...
set; other bound or UDF matrix-result receivers do not enter this path.
