...
`.eigenvalues()`, `.is_zero()`, `.is_binary()`, `.is_diagonal()`, and `.is_identity()`, plus all-kind terminal `.is_square()`. Int inputs still produce
...
continuation. `.is_square()` returns the ordinary simple bool for every
...
upstream-`na` propagation, and is terminal. Other terminal readers, invalid
arity or argument types, wrong-result helpers, broader postfix helpers, and
...
matrix kind through `SameAsArg`, and admit the same thirteen matrix helpers subject
to the numeric eigenvalue and value-predicate checks, with the same matrix-copy/
array-result and terminal-predicate-reader continuation rules. Exact namespace
