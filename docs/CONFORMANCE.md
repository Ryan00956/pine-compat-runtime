...
creating new ones. `line.new` can consume two `chart.point` values through its
point overload. Typed declarations, drawing point overloads beyond `line.new`,
and general polyline arrays remain outside the supported subset; see
...
chart.point          partial      chart.point.new/now/from_index/from_time/copy constructors, time/index/price field reads, top-level field mutation, and array.new<chart.point>/array.from chart-point array storage/read/mutation/search are fixture-backed; line.new consumes two chart.point values through its point overload, and polyline.new consumes chart-point arrays through its dedicated partial row, including default/declaration-driven max-count eviction; typed declarations, drawing point overloads beyond line.new, and general polyline arrays remain unsupported
