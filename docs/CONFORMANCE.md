...
`.elements_count()`, `.get(row, column)`, `.copy()`, and `.transpose()`.
Unqualified local-UDF results that infer a concrete supported matrix kind use
that same six-helper matrix set through `$call_result`; parameter passthrough,
block aliases, nested calls, same-kind control flow, matrix operations, and
constructors retain call-specific float/int/bool/string/color kinds. Both
`.copy()` and `.transpose()` may continue. Local and imported user-method
results with a concrete supported matrix kind share the six helpers through recorded method-call
...
matrix kind also share the six helpers across alias-qualified, block/nested/
...
type-compatible initial value or default `na`, and the same six direct
read/copy/transpose helpers with copy/transpose continuation.
...
matrix results allow `.copy()` or `.transpose()` to continue the matrix chain.
...
matrix kind through `SameAsArg`, and admit the same twenty-five matrix helpers subject
...
default full bounds and empty row/column slices, and shares only the same six
...
the same six helpers. Exact namespace `matrix.pow(values, power)` returns a
fixed square `matrix<float>` for numeric matrices and simple-int powers,
preserves identity/copy/positive-power semantics, and admits only those six
...
the same six helpers. Exact namespace `matrix.pinv(values)` returns an
...
while admitting only the same six helpers. Exact namespace
...
non-real, or incomplete results, while admitting only the same six helpers.
...
cells, fresh allocation, and copy independence, and admit only the same six
...
`.transpose()` with copy/transpose continuation when `matrix_id` has a
...
and keep copy/transpose continuation. Direct helpers on bound
numeric `matrix_id.kron(other)` results admit the same helpers, expand both
dimensions, resolve to independent `matrix<float>` storage, and keep copy/transpose
...
also admit that set with copy/transpose continuation and call-specific result kinds.
...
block/nested/control-flow, and copy/transpose-continuation paths. Remaining
...
`.rows()`/`.columns()`/`.elements_count()`/`.get(row, column)`/`.copy()`/
`.transpose()` set for matrix results. `matrix.copy` always takes the matrix
...
array-result chain; matrix results may continue through `.copy()` or
`.transpose()`. Exact bound matrix-receiver `values.copy()`
results preserve element kind, shape, and independent storage and admit the
same six helpers. Exact bound matrix-receiver `values.transpose()` results
...
receivers instead enter the same six-helper set only for unqualified local
...
zero-dimension, dual-alias, independent-copy, and copy/transpose-continuation paths.
...
kind, zero-dimension, dual-alias, independent-copy, and copy/transpose-continuation
