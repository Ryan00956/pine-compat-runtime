...
plus fixture-backed `box.get_top` over the latest existing box snapshot, with
...
and shares the box runtime limit. `box.get_top` reads the latest existing box
snapshot and returns `na` for `na` or deleted boxes; later box getters remain
