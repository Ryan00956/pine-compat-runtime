...
`.clear()`, and `.remove(key)`, `.get(key)`, `.contains(key)`, `.copy()`,
...
template share those nine helpers through `$call_result`; parameter
...
other than terminal `.put(...)`/`.clear()`/`.remove(...)`, array mutation outside the admitted derived-
...
admit `.size()`, terminal `.put(key, value)`, `.clear()`, and `.remove(key)`, `.get(key)`, `.contains(key)`,
`.copy()`, `.keys()`, and `.values()`. Only `.copy()` may continue another
admitted map helper; `.put(...)`, `.clear()`, and `.remove(...)` return `void`
and cannot continue.
...
mutation other than terminal `.put(...)`/`.clear()`/`.remove(...)`, unsupported templates, non-map copy
...
also share those nine helpers and copy-only continuation through `$call_result`;
...
isolation, clear-to-empty and existing/missing-key remove alias/fresh behavior,
