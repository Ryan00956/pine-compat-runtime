...
`array<float>`, `array<bool>`, `array<string>`, and `array<color>`
declarations, are fixture-backed with compatible or `na` initializers and
later compatible reassignment. Bare `array`, non-scalar array element types,
UDT, map, matrix, and other typed declarations remain unsupported unless a
narrower feature row explicitly backs them with fixtures.
...
typed declarations   partial      int, float, bool, string, color, chart.point, drawing-id label/line/linefill/box/table/polyline, and scalar array<int>/array<float>/array<bool>/array<string>/array<color> typed variable declarations with compatible or na initializers and later compatible reassignment are fixture-backed; bare array, non-scalar array element types, UDT, map, matrix, and other typed declarations remain unsupported unless covered by narrower feature rows
