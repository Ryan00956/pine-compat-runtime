...
**发现的问题（仅记录，未修改）**
- [中] `ta.*` 隐式历史需求已从 `history.rs` 手写 callee 表迁到 [pine-builtins history metadata](crates/pine-builtins/src/history.rs)，`history.rs::record_call_history` 改为消费该共享声明，且 `infers_implicit_ta_history_requirements_by_series` 覆盖主要隐式 `ta.*` 表项。剩余风险是 runtime 对应实现仍独立读取历史（如 `previous_close`/`builtin_f64_at`/`series_store.read`）；若实现漂移而 metadata 未同步，仍可能**静默低估**历史缓冲区。建议继续做 runtime 读路径与 metadata 对账，并补端到端数值基准。— [history.rs](crates/pine-sema/src/history.rs)
...
| **P1-c** | `ta.*` 隐式历史回看表与 runtime 强耦合，漂移静默低估缓冲区 | 中 | 3/6 | 已引入 `pine-builtins` 共享 history metadata 并让 sema 消费；后续仍需 runtime 实现对账 + 端到端数值基准 |
...
- [阶段3][中] crates/pine-sema/src/history.rs / crates/pine-builtins/src/history.rs — ta.* 隐式历史需求已迁到共享 metadata 并由 sema 消费；runtime 实现仍独立读取历史，漂移仍会静默低估历史缓冲区 — 后续补 runtime/metadata 对账与端到端数值基准（阶段5/6 对账）。
...
- [阶段6][中] crates/pine-runtime/src/runtime/context.rs — 运行时侧确认阶段3历史深度耦合：series_retention.max_depth_for 源自 sema series_history，低估则 series_store.read(offset>len) 静默返回 Na（错误结果非崩溃） — 阶段3已把 sema 需求统一到 `pine-builtins` metadata；后续补 runtime/metadata 对账与端到端回归。
