...
**发现的问题（仅记录，未修改）**
- [中] `ta.*` 隐式历史需求已从 `history.rs` 手写 callee 表迁到 [pine-builtins history metadata](crates/pine-builtins/src/history.rs)，`history.rs::record_call_history` 改为消费该共享声明，且 `infers_implicit_ta_history_requirements_by_series` 覆盖主要隐式 `ta.*` 表项。当前已补 `runtime_implicit_history_calls_match_shared_metadata`，用 reviewed runtime 隐式历史读取清单与共享 metadata 做完全对账；`runs_sar_over_historical_bars`、`runs_dmi_over_historical_bars`、`runs_supertrend_over_historical_bars`、`runs_keltner_channels_over_historical_bars`、`runs_keltner_channel_width_over_historical_bars`、`runs_mfi_over_historical_bars` 与 `runs_tsi_over_historical_bars` 也已把端到端数值序列分别绑定到 `high[2]/low[2]/close[1]`、`high[1]/low[1]/close[1]`、`close[1]`、`close[1]`、`close[1]`、`close[1]` 与 `close[1]` HIR retention 断言。剩余风险是 runtime 读路径仍非编译期自动绑定，且高风险指标数值基准仍不完整；若实现漂移但清单/metadata 未同步，仍可能**静默低估**历史缓冲区。建议继续补 `ta.cross*` 的 oracle/golden 数值回归。— [history.rs](crates/pine-sema/src/history.rs)
...
| **P1-c** | `ta.*` 隐式历史回看表与 runtime 强耦合，漂移静默低估缓冲区 | 中 | 3/6 | 已引入 `pine-builtins` 共享 history metadata、sema 消费、runtime reviewed-list 对账与 `ta.sar`/`ta.dmi`/`ta.supertrend`/`ta.kc`/`ta.kcw`/`ta.mfi`/`ta.tsi` retention+数值基准；后续仍需扩展其他高风险指标数值基准 |
