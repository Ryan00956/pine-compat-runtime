...
| **P1-c** | `ta.*` 隐式历史回看表与 runtime 强耦合，漂移静默低估缓冲区 | 中 | 3/6 | 已引入 `pine-builtins` 共享 history metadata、sema 消费、runtime reviewed-list 对账与 `ta.sar`/`ta.dmi` retention+数值基准；后续仍需扩展其他高风险指标数值基准 |
...
- [阶段6][中] crates/pine-runtime/src/runtime/context.rs — 运行时侧确认阶段3历史深度耦合：series_retention.max_depth_for 源自 sema series_history，低估则 series_store.read(offset>len) 静默返回 Na（错误结果非崩溃） — 阶段3已把 sema 需求统一到 `pine-builtins` metadata，补 reviewed runtime/metadata 对账，并把 `ta.sar`/`ta.dmi` 数值基准绑定到 HIR retention；后续补其他高风险指标端到端回归。
