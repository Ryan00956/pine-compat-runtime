...
| **P1-c** | `ta.*` 隐式历史回看表与 runtime 强耦合，漂移静默低估缓冲区 | 中 | 3/6 | 已引入 `pine-builtins` 共享 history metadata、sema 消费、runtime reviewed-list 对账与 `ta.sar`/`ta.dmi`/`ta.supertrend` retention+数值基准；后续仍需扩展其他高风险指标数值基准 |
...
- [阶段3][中] crates/pine-sema/src/history.rs / crates/pine-builtins/src/history.rs / crates/pine-runtime/src/tests/{builtin_registry,builtins_ta_flow}.rs — ta.* 隐式历史需求已迁到共享 metadata、由 sema 消费，并与 reviewed runtime 隐式历史读取清单对账；`ta.sar`、`ta.dmi` 与 `ta.supertrend` 已有绑定 HIR retention 的端到端数值基准；runtime 读路径仍非编译期自动绑定，漂移仍可能静默低估历史缓冲区 — 后续扩展其他高风险指标数值基准（阶段5/6 对账）。
