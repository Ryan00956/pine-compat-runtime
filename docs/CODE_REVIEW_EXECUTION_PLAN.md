...
| **P1-c** | `ta.*` 隐式历史回看表与 runtime 强耦合，漂移静默低估缓冲区 | 中 | 3/6 | 已引入 `pine-builtins` 共享 history metadata、sema 消费、runtime reviewed-list 对账与 `ta.sar`/`ta.dmi`/`ta.supertrend`/`ta.kc`/`ta.kcw` retention+数值基准；后续仍需扩展其他高风险指标数值基准 |
