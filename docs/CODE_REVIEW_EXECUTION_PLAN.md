...
**发现的问题（仅记录，未修改）**
- [中] `ta.*` 隐式历史需求已从 `history.rs` 手写 callee 表迁到 [pine-builtins history metadata](crates/pine-builtins/src/history.rs)，`history.rs::record_call_history` 改为消费该共享声明，且 `infers_implicit_ta_history_requirements_by_series` 覆盖主要隐式 `ta.*` 表项。当前已补 `runtime_implicit_history_calls_match_shared_metadata`，用 reviewed runtime 隐式历史读取清单与共享 metadata 做完全对账；`runs_sar_over_historical_bars`、`runs_dmi_over_historical_bars`、`runs_supertrend_over_historical_bars`、`runs_keltner_channels_over_historical_bars`、`runs_keltner_channel_width_over_historical_bars`、`runs_mfi_over_historical_bars`、`runs_tsi_over_historical_bars` 与 `runs_cross_functions_over_historical_bars` 也已把端到端数值序列绑定到对应 HIR retention 断言；cross helper 覆盖 `close[1]` 与 series `baseline[1]`。剩余风险是 runtime 读路径仍非编译期自动绑定；若实现漂移但清单/metadata 未同步，仍可能**静默低估**历史缓冲区。下一步建议评估 runtime helper/debug assertion 是否值得进入 P1-c closeout。— [history.rs](crates/pine-sema/src/history.rs)
...
| **P1-c** | `ta.*` 隐式历史回看表与 runtime 强耦合，漂移静默低估缓冲区 | 中 | 3/6 | 已引入 `pine-builtins` 共享 history metadata、sema 消费、runtime reviewed-list 对账与 `ta.sar`/`ta.dmi`/`ta.supertrend`/`ta.kc`/`ta.kcw`/`ta.mfi`/`ta.tsi`/`ta.cross*` retention+数值基准；后续评估 runtime helper/debug assertion closeout |
...
- [阶段3][中] crates/pine-sema/src/history.rs / crates/pine-builtins/src/history.rs / crates/pine-runtime/src/tests/{builtin_registry,builtins_ta_flow,builtins_ta_averages}.rs — ta.* 隐式历史需求已迁到共享 metadata、由 sema 消费，并与 reviewed runtime 隐式历史读取清单对账；`ta.sar`、`ta.dmi`、`ta.supertrend`、`ta.kc`、`ta.kcw`、`ta.mfi`、`ta.tsi` 与 `ta.cross*` 已有绑定 HIR retention 的端到端数值基准；runtime 读路径仍非编译期自动绑定，漂移仍可能静默低估历史缓冲区 — 后续评估 runtime helper/debug assertion closeout（阶段5/6 对账）。
...
- [阶段6][中] crates/pine-runtime/src/runtime/context.rs — 运行时侧确认阶段3历史深度耦合：series_retention.max_depth_for 源自 sema series_history，低估则 series_store.read(offset>len) 静默返回 Na（错误结果非崩溃） — 阶段3已把 sema 需求统一到 `pine-builtins` metadata，补 reviewed runtime/metadata 对账，并把 `ta.sar`/`ta.dmi`/`ta.supertrend`/`ta.kc`/`ta.kcw`/`ta.mfi`/`ta.tsi`/`ta.cross*` 数值基准绑定到 HIR retention；后续评估 runtime helper/debug assertion closeout。
