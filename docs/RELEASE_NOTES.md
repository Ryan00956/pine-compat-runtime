...
- Added fixture-backed scalar array variable history snapshots for Pine-style
  `previous = a[1]` and `na(previous) ? na : previous.get(0)` reads, with
  retained array values copied into history and positive-offset history reads
  returning a fresh array copy.
