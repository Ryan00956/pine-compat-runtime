...
- Added fixture-backed UDT passthrough through UDF parameters, UDF returns, and
  ...
  maps back to one of those method parameters, or construct and return a local
  UDT from receiver scalar fields, and allow the caller to assign and field-read
  ...
  return local UDT values from inferred scalar parameters; field mutation
  inside functions or methods, imported UDT identity, UDT history, `varip`,
