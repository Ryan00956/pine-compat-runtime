...
Table entries
carry `position`, `bgColor`, `columns`, `rows`, and sparse cell snapshots. Each table cell
...
table's final position, `table.set_bgcolor` updates the table's final
background color, while `table.cell_set_text`,
