...
GitHub Actions builds the Python extension wheels for the first supported
desktop platforms:
...
free-threaded CPython are outside the first release matrix.
...
- `crates/pine-python/Cargo.toml` package version
...
3. Create and push the annotated tag:
   ...
   ```text
   git tag -a v0.1.0 -m "Pine Compat Runtime v0.1.0"
