...
fn validate_builtin_namespace_matrix_call_result_fixture_paths(
    ...
    require_fixtures(
        ...
        "fixture-backed exact namespace matrix.mult/matrix.copy/matrix.transpose/matrix.submatrix/matrix.kron/matrix.diff/matrix.pow/matrix.inv/matrix.pinv/matrix.eigenvectors result rows/columns/elements_count/get/copy dispatch and retained result-type/helper/bound-receiver boundaries",
    )
}
