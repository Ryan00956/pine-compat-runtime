...
plot(na(missing_int) and na(missing_float) and not missing_bool and na(missing_string) and na(missing_color) and na(missing_box) and na(missing_label) and na(missing_line) and na(missing_linefill) ? 1 : 0)
"#,
    );
    let analysis = analyze_source(&source);
    assert!(
