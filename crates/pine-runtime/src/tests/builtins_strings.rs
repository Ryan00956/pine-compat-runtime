...
plot(upper == "SMA" and lower == "sma" and unicode_length == 2 and empty_length == 0 ? length : 0)
plot(na(missing) and na(missing_upper) and na(missing_lower) ? 1 : 0)
plot(matched and not_matched and empty_match ? 1 : 0)
...
plot(na(na_pos) and na(na_source_pos) ? 1 : 0)
plot(na(missing_pos) ? 1 : 0)
plot(slice == "M" and tail == "MA" and wide == "MA" and empty_slice == "" and na(na_source_slice) and na_begin == "S" and na_end == "MA" and unicode_slice == "β" ? 1 : 0)
plot(trimmed == upper and na(missing_trim) and repeated == "ab-ab" and default_repeat == "abab" and empty_repeat == "" ? 1 : 0)
...
plot(formatted_array == "Values [1.2, 2.6, NaN]" and formatted_datetime == "2021-01-01T00:00:00+0000" and formatted_quote == "Literal {0} and apostrophe ' X" and na(formatted_na) and formatted_na_arg == "Missing NaN" and formatted_bool == "Flag true" ? 1 : 0)
