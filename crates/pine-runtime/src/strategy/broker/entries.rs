...
impl BrokerState {
    ...
    pub(super) fn entry_long_internal(
        ...
        if pyramiding_mode == EntryPyramidingMode::EnforceLimit && self.position_size < 0.0 {
            self.close_all_long(fill.bar_index, fill.time, fill.price);
        }
    ...
    pub(super) fn entry_short_internal(
        ...
        if matches!(
            ...
            self.record_open_short_trade(open_trade);
        }
        self.record_order_event(
            fill.id,
            fill.bar_index,
    ...
    fn record_open_short_trade(&mut self, open_trade: OpenTrade) {
        self.record_open_long_legacy_state(&open_trade);
        if self.pyramiding_limit <= 1 {
    ...
    #[allow(dead_code)]
    fn record_open_short_trade_exceeding_pyramiding(&mut self, open_trade: OpenTrade) {
        self.record_open_long_legacy_state(&open_trade);
        self.trade_ledger.append_short(open_trade);
        self.sync_aggregate_position_from_ledger();
    }
