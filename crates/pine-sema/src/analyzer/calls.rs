    ...
    fn analyze_matrix_call_result_method(
        ...
        let Some(builtin_name) = matrix_call_result_builtin_name(method_name) else {
            self.unsupported(
                ...
                "direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
                callee.span,
            );
            return Some(None);
        };
