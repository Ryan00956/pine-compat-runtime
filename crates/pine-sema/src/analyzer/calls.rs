    ...
    fn analyze_map_call_result_method(
        ...
        let Some(builtin_name) = map_call_result_builtin_name(method_name) else {
            self.unsupported(
                ...
                "direct map call-result methods currently support only `.size()`, terminal `.put()`, `.clear()`, and `.remove()`, `.get()`, `.contains()`, `.copy()`, `.keys()`, and `.values()`; bind the result or use the namespace helper",
                callee.span,
            );
            return Some(None);
        };
