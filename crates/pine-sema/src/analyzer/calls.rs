    ...
    fn analyze_array_call_result_method(
        ...
        let Some(builtin_name) = array_call_result_builtin_name(method_name) else {
            self.unsupported(
                ...
                "direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, `.binary_search()`, `.binary_search_leftmost()`, and `.binary_search_rightmost()`; bind the result or use the namespace helper",
                callee.span,
            );
            return Some(None);
        };
