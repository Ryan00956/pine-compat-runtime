...
fn accepts_provider_backed_same_timeframe_request_security_ta() {
    let analysis = analyze(
        "plot(request.security(\"NYSE:IBM\", timeframe.period, ta.sma(close, 2) + ta.cum(close) + ta.rsi(close, 3) + ta.tr() + nz(ta.tr(false)) + ta.atr(3) + ta.highest(high, 3) - ta.lowest(low, 3) + ta.change(close) + ta.mom(close, 2) + ta.roc(close, 2) + ta.range(close, 3) + ta.dev(close, 3) + ta.bbw(close, 3, 2) + ta.correlation(close, high, 3) + ta.stdev(close, 3) + ta.stdev(close, 3, false) + ta.variance(close, 3) + ta.variance(close, 3, false) + ta.wma(close, 3) + ta.swma(close) + ta.hma(close, 4) + ta.alma(close, 4, 0.85, 6) + ta.linreg(close, 3, 0) + (ta.rising(close, 2) ? 1 : 0) - (ta.falling(close, 2) ? 1 : 0) + (ta.cross(close, 20.5) ? 1 : 0) + (ta.crossover(close, 20.5) ? 1 : 0) - (ta.crossunder(close - time / 60000.0, 19.5) ? 1 : 0)))\n",
    );
