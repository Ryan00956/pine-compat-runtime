...
fn reports_unsupported_local_user_type_array_call_result_chaining_fixture() {
    ...
    assert_diagnostic_messages(
        ...
        &[
            ...
            "`array.pop` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.transform` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "ternary UDT array branches must resolve to the same element identity",
            "ternary UDT array branches must resolve to the same element identity",
            "ternary UDT array branches must resolve to the same element identity",
            "ternary UDT array branches must resolve to the same element identity",
    ...
    assert_diagnostic_count(path, 39);
}
...
fn reports_unsupported_builtin_namespace_array_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            "`array.get` argument `index` expects simple integer-compatible, got const string",
            "`array.pop` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.push` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.get` argument `index` expects simple integer-compatible, got const string",
            "`array.clear` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            ...
            "`array.get` argument `index` expects simple integer-compatible, got const string",
            "`array.get` argument `index` expects simple integer-compatible, got const string",
            "`array.reverse` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.get` argument `index` expects simple integer-compatible, got const string",
            "`array.get` argument `index` expects simple integer-compatible, got const string",
            "`array.remove` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.set` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.get` argument `index` expects simple integer-compatible, got const string",
            "`array.pop` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.reverse` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
    ...
    assert_diagnostic_count(path, 46);
}
...
fn reports_unsupported_bound_matrix_mult_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            ...
            "`matrix.set` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.diff()`, `.eigenvectors()`, `.inv()`, `.kron()`, `.mult()`, `.pinv()`, `.pow()`, `.submatrix()`, `.transpose()`, `.row()`, `.col()`, `.eigenvalues()`, `.is_square()`, `.is_zero()`, `.is_binary()`, `.is_diagonal()`, `.is_identity()`, `.is_symmetric()`, `.is_antisymmetric()`, `.is_stochastic()`, `.sum()`, `.avg()`, `.min()`, `.max()`, `.mode()`, `.trace()`, `.det()`, and `.rank()`; bind the result or use the namespace helper",
            "`matrix.size` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.diff()`, `.eigenvectors()`, `.inv()`, `.kron()`, `.mult()`, `.pinv()`, `.pow()`, `.submatrix()`, `.transpose()`, `.row()`, `.col()`, `.eigenvalues()`, `.is_square()`, `.is_zero()`, `.is_binary()`, `.is_diagonal()`, `.is_identity()`, `.is_symmetric()`, `.is_antisymmetric()`, `.is_stochastic()`, `.sum()`, `.avg()`, `.min()`, `.max()`, `.mode()`, `.trace()`, `.det()`, and `.rank()`; bind the result or use the namespace helper",
            "`array.rows` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
...
fn reports_unsupported_local_udf_map_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            ...
            "`array.push` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.push` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.contains` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`call_result.copy` is not supported: direct call-result methods require a supported concrete receiver type; bind the result first",
            "`call_result.size` is not supported: direct call-result methods require a supported concrete receiver type; bind the result first",
            "`call_result.size` is not supported: direct call-result methods require a supported concrete receiver type; bind the result first",
            "`array.contains` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
...
fn reports_unsupported_builtin_namespace_matrix_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            ...
            "`matrix.size` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.diff()`, `.eigenvectors()`, `.inv()`, `.kron()`, `.mult()`, `.pinv()`, `.pow()`, `.submatrix()`, `.transpose()`, `.row()`, `.col()`, `.eigenvalues()`, `.is_square()`, `.is_zero()`, `.is_binary()`, `.is_diagonal()`, `.is_identity()`, `.is_symmetric()`, `.is_antisymmetric()`, `.is_stochastic()`, `.sum()`, `.avg()`, `.min()`, `.max()`, `.mode()`, `.trace()`, `.det()`, and `.rank()`; bind the result or use the namespace helper",
            "`array.rows` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.columns` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.elements_count` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
...
fn reports_unsupported_builtin_array_call_result_reads_fixture() {
    ...
    assert_diagnostic_messages(
        ...
        &[
            ...
            "`array.transform` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.push` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.slice` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
            "`array.clear` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, `.copy()`, `.includes()`, `.indexof()`, `.lastindexof()`, and `.binary_search()`; bind the result or use the namespace helper",
    ...
    assert_diagnostic_count(path, 37);
}
...
fn reports_unsupported_imported_user_type_array_call_result_chaining_fixture() {
    ...
    assert_import_diagnostic_count_with_library(path, "user/udt_array_returns/1", library, 25);
}
