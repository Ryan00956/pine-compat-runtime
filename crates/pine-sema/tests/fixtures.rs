...
fn reports_unsupported_bound_matrix_pow_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            ...
            "`matrix.set` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, `.eigenvalues()`, `.is_square()`, `.is_zero()`, `.is_binary()`, `.is_diagonal()`, `.is_identity()`, `.is_symmetric()`, `.is_antisymmetric()`, `.is_stochastic()`, `.sum()`, `.avg()`, `.min()`, `.max()`, `.mode()`, and `.trace()`; bind the result or use the namespace helper",
...
fn reports_unsupported_builtin_namespace_matrix_call_result_reads_fixture() {
    ...
    assert_diagnostic_count(path, 88);
}
