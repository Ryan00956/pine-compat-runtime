...
fn reports_unsupported_bound_matrix_pow_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            ...
            "`matrix.set` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
            "`matrix.sum` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
            "`matrix.size` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
...
fn reports_unsupported_bound_matrix_transpose_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            ...
            "`matrix.get` argument `column` expects simple int, got const string",
            "`matrix.set` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
            "`matrix.reverse` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
            "`matrix.sum` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
            "`matrix.size` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
...
fn reports_unsupported_local_udf_map_call_result_reads_fixture() {
    ...
    assert_exact_diagnostic_messages(
        ...
        &[
            ...
            "`call_result.size` is not supported: direct call-result methods require a supported concrete receiver type; bind the result first",
            "`array.contains` is not supported: direct array call-result methods currently support only `.size()`, `.get()`, `.first()`, `.last()`, and `.copy()`; bind the result or use the namespace helper",
            "`matrix.contains` is not supported: direct matrix call-result methods currently support only `.rows()`, `.columns()`, `.elements_count()`, `.get()`, `.copy()`, `.row()`, `.col()`, and `.eigenvalues()`; bind the result or use the namespace helper",
        ],
    );
